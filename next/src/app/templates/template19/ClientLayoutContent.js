...
import FloatingButtons from '@/dynamic-components/template19/components/FloatingButtons/FloatingButtons';
import Footer from '@/dynamic-components/template19/components/Footer/Footer';
import GlobalPageLoader from '@/dynamic-components/template19/components/GlobalPageLoader/GlobalPageLoader';
import Navbar from '@/dynamic-components/template19/components/Navbar/Navbar';
...
const LayoutContent = memo(function LayoutContent({ children }) {
  ...
  const mainClassName = useMemo(
    () => `flex-1 template19-main ${isMobilePlatform ? 'pb-safe-bottom' : ''}`,
    [isMobilePlatform],
  );

  return (
    <div className="template19-layout flex min-h-screen flex-col">
      <div className="relative flex w-full min-w-0 flex-1 flex-col overflow-x-hidden lg:w-auto">
        <div className="template19-header z-50">
          <Navbar />
        </div>
...
const ClientLayoutContent = memo(function ClientLayoutContent({ children }) {
  return (
    <div className="template19-app antialiased">
      <ThemeProvider
        attribute="class"
        defaultTheme="dark"
        enableSystem={true}
