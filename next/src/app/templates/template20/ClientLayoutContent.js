...
import FloatingButtons from '@/dynamic-components/template20/components/FloatingButtons/FloatingButtons';
import Footer from '@/dynamic-components/template20/components/Footer/Footer';
import GlobalPageLoader from '@/dynamic-components/template20/components/GlobalPageLoader/GlobalPageLoader';
import Navbar from '@/dynamic-components/template20/components/Navbar/Navbar';
...
const LayoutContent = memo(function LayoutContent({ children }) {
  ...
  const mainClassName = useMemo(
    () => `flex-1 template20-main ${isMobilePlatform ? 'pb-safe-bottom' : ''}`,
    [isMobilePlatform],
  );

  return (
    <div className="template20-layout flex min-h-screen flex-col">
      <div className="relative flex w-full min-w-0 flex-1 flex-col overflow-x-hidden lg:w-auto">
        <div className="template20-header z-50">
          <Navbar />
        </div>
...
const ClientLayoutContent = memo(function ClientLayoutContent({ children }) {
  return (
    <div className="template20-app antialiased">
      <ThemeProvider
        attribute="class"
        defaultTheme="dark"
        enableSystem={true}
