...
import FloatingButtons from '@/dynamic-components/template12/components/FloatingButtons/FloatingButtons';
import Footer from '@/dynamic-components/template12/components/Footer/Footer';
import GlobalPageLoader from '@/dynamic-components/template12/components/GlobalPageLoader/GlobalPageLoader';
import Navbar from '@/dynamic-components/template12/components/Navbar/Navbar';
...
const LayoutContent = memo(function LayoutContent({ children }) {
  ...
  const mainClassName = useMemo(
    () => `flex-1 template12-main ${isMobilePlatform ? 'pb-safe-bottom' : ''}`,
    [isMobilePlatform],
  );

  return (
    <div
      className="template12-layout flex min-h-screen flex-col"
      style={{ backgroundColor: '#000000' }}
    >
      <div className="template12-header z-50">
        <Navbar />
      </div>
...
const ClientLayoutContent = memo(function ClientLayoutContent({ children }) {
  return (
    <div
      className="template12-app antialiased"
      style={{ backgroundColor: '#000000' }}
    >
      <ThemeProvider
        attribute="class"
