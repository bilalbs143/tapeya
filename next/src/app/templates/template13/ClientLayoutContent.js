...
import FloatingButtons from '@/dynamic-components/template13/components/FloatingButtons/FloatingButtons';
import Footer from '@/dynamic-components/template13/components/Footer/Footer';
import GlobalPageLoader from '@/dynamic-components/template13/components/GlobalPageLoader/GlobalPageLoader';
import LeftSidebar from '@/dynamic-components/template13/components/LeftSidebar/LeftSidebar';
import Navbar from '@/dynamic-components/template13/components/Navbar/Navbar';
import RightSidebar from '@/dynamic-components/template13/components/RightSidebar/RightSidebar';
...
const LayoutContent = memo(function LayoutContent({ children }) {
  ...
  const mainClassName = useMemo(
    () => `flex-1 template13-main ${isMobilePlatform ? 'pb-safe-bottom' : ''}`,
    [isMobilePlatform],
  );

  return (
    <div
      className="template13-layout flex min-h-screen flex-col"
      style={{ backgroundColor: '#001724' }}
    >
      <div className="template13-header z-50">
        <Navbar />
        {/* <SubNavbar /> */}
      </div>
...
const ClientLayoutContent = memo(function ClientLayoutContent({ children }) {
  return (
    <div
      className="template13-app antialiased"
      style={{ backgroundColor: '#001724' }}
    >
      <ThemeProvider
        attribute="class"
