import '@/app/styles/template11.css';
...
const templateConfig = getTemplateConfig(TEMPLATE_NAMES.TEMPLATE11);

// Template 11 uses Urbanist font
...
const kingTown = localFont({
  src: '../../fonts/template11/king-town.otf',
  variable: '--font-king-town',
  display: 'swap',
});

// Template 11 specific metadata
export const metadata = {
  ...
  other: {
    template: TEMPLATE_NAMES.TEMPLATE11,
    'template-name': templateConfig.name,
  },
};

// Template 4 specific viewport
...
export default function Template4Layout({ children }) {
  return (
    <html lang="en" suppressHydrationWarning={true} className="template4-html">
      <head>
        ...
        <meta name="template" content={TEMPLATE_NAMES.TEMPLATE11} />
        <meta name="template-name" content={templateConfig.name} />
      </head>
      <body
        className={`${urbanist.className} ${urbanist.variable} ${kingTown.className} ${kingTown.variable} template11-app template11-body antialiased`}
      >
