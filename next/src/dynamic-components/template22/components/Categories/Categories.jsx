...
const CategoryItem = React.memo(function CategoryItem({ category, isMobile, label, isEnabled, href }) {
  const content = (
    <>
      <div
  ...
  const boxStyle = isMobile
    ? {
      background:
          'linear-gradient(90deg, rgba(26,26,26,0.3) 0%, rgba(0,0,0,0.3) 10%, rgba(0,0,0,0.3) 100%), #2e2e50',
    }
    : undefined;
  ...
  if (isEnabled) {
    return (
      <Link
        href={effectiveHref}
        className={`group flex flex-shrink-0 flex-col items-center transition-all duration-200 hover:opacity-80 active:scale-95 ${
          isMobile ? 'min-w-[56px] gap-1.5 rounded py-2 px-1' : 'gap-2 sm:gap-2.5'
        }`}
        style={boxStyle}
      >
        {content}
...
function Categories() {
  ...
  const categoriesContent = useMemo(() => {
    return (
      <div className="flex items-baseline gap-2 sm:gap-2 md:gap-4 lg:gap-8" style={{ width: 'max-content', marginLeft: 'auto' }}>
        {CATEGORIES.map((category) => {
          ...
          if (isDesktop && CATEGORIES_WITH_PREVIEW.includes(category.key)) {
            return (
              ...
              >
                <CategoryItem category={category} isMobile={false} label={label} isEnabled={isEnabled} href={href} />
              </div>
            );
          }
          return (
            <CategoryItem key={category.key} category={category} isMobile={false} label={label} isEnabled={isEnabled} href={href} />
          );
        })}
      </div>
    );
  }, [isDesktop, getCategoryLabelAndHref, handleCategoryHoverEnter, handleCategoryHoverLeave]);

  const mobileCategoriesContent = useMemo(() => {
    return (
      <div className="flex items-center gap-3 px-1" style={{ width: 'max-content' }}>
        {CATEGORIES.map((category) => {
          const { label, href } = getCategoryLabelAndHref(category);
          const isEnabled = ENABLED_CATEGORY_KEYS.includes(category.key);
          return (
            <CategoryItem key={category.key} category={category} isMobile label={label} isEnabled={isEnabled} href={href} />
          );
        })}
      </div>
    );
  }, [getCategoryLabelAndHref]);
  ...
  return (
    <section className="bg-[#2e2e50] md:bg-black">
      <div className="container mx-auto px-0 py-2 sm:px-4 sm:py-3">
          ...
          <div className="flex items-center gap-3 sm:gap-6 md:gap-10 lg:gap-14">
            ...
            <div className="flex min-w-0 flex-1 items-center md:hidden">
              <button
                ...
                className="grid h-10 w-7 flex-shrink-0 place-items-center bg-[#2e2e50] text-white transition-opacity hover:opacity-90 active:opacity-80"
              ...
              <div
                ref={mobileSliderRef}
                className="categories-slider scrollbar-hide flex min-w-0 flex-1 items-center overflow-x-auto overflow-y-hidden"
              >
