  ...
  const Section = ({ items }) => (
    <div className="space-y-4">
      {items.map((item, idx) => (
        ...
        >
          <div className="mobile-faq-question bg-[#E8D25E] bg-clip-text px-4 py-3 font-semibold text-transparent">
            {item.q}
          </div>
  ...
  return (
    <div className="min-h-screen text-white">
      <div className="container mx-auto px-4 py-8">
        <h1 className="mobile-title mb-2 mb-8 bg-[#E8D25E] bg-clip-text text-[25px] font-semibold text-transparent">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`mobile-tab-button rounded-lg px-3 py-2 text-sm transition-colors sm:px-4 sm:py-3 sm:text-base ${
                  activeTab === tab.key
                    ? 'bg-[#E8D25E] font-semibold text-black'
                    : 'border border-[#E8D25E] text-[#E8D25E] hover:bg-[#E8D25E] hover:font-semibold hover:text-black'
                }`}
              >
                {tab.label}
              </button>
