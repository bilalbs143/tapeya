...
const CryptoPaymentDetails = ({ paymentData, onClose }) => {
  ...
  const renderAmountDisplay = () => (
    <div className="rounded-[5px] border border-[#182F31] bg-[#131515] px-2 py-4 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
      <div className="text-center text-base font-bold md:text-[25px]">
        <span className="font-normal text-[#E33A24]">
          {displayAmount.amount}
        </span>{' '}
        <span className="font-bold text-[#E33A24]">
  ...
  const renderQRCode = () => {
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <div className="mt-3 text-xs tracking-wide text-[#E33A24] uppercase">
          {t('scan_to_pay')}
        </div>
      </div>
    );
  ...
  const renderAddressSection = () => (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
      <div className="flex h-[46px] w-full items-center rounded-[3px] border border-[#182F31] bg-transparent px-3 text-[12px] text-white md:text-[14px]">
      ...
      <button
        className="template10-filled-button-hover w-full rounded-[4px] bg-[#E33A24] px-5 py-3 text-sm font-bold text-black transition-all duration-200 active:scale-95 sm:w-[270px]"
        onClick={() => copyToClipboard(paymentData.pay_address)}
      >
  ...
  return (
    <div className="mt-4 rounded-[5px] border border-[#182F31] bg-[#1D4647] p-5">
      {/* Header */}
      <div className="mb-6 flex items-center justify-start gap-4">
        <button
          onClick={onClose}
          aria-label={t('back')}
          className="template10-filled-button-hover flex items-center justify-center rounded-[4px] bg-[#E33A24] px-12 py-4 font-normal text-white transition-all duration-300 md:ml-3"
        >
          <span className="text-container">
            <span className="text">Back</span>
          </span>
        </button>
        <span className="text-[15px] font-bold text-white md:ml-2 md:text-[15px]">
          {t('deposit_details')}
        </span>
      </div>

      {/* Amount and QR Section */}
      <div className="grid grid-cols-1 items-start gap-4 md:ml-3 md:grid-cols-3">
        <div className="md:col-span-2">
          <div className="mb-2 text-[10px] font-bold text-white">
            {t('amount_to_pay')}
          </div>
          <div className="rounded-[10px] border border-[#182F31] bg-transparent px-2 py-2 text-white">
