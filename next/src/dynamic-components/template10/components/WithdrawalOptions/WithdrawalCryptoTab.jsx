...
import CryptoSelect from '@/dynamic-components/template10/components/CryptoSelect/CryptoSelect';
import CryptoWithdrawalDetails from '@/dynamic-components/template10/components/CryptoWithdrawalDetails/CryptoWithdrawalDetails';
...
export default function WithdrawalCryptoTab() {
  ...
  const verifyButtonClassName = useMemo(() => {
    ...
    return `${baseClasses} bg-[#E33A24] text-white   font-semibold   hover:shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]`;
  }, [addressVerificationStatus]);
  ...
  const renderExchangeRateDisplay = () => {
    ...
    return (
      <div className="mt-2 rounded-[3px] border border-[#E33A2480] bg-[#182F31] p-3 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
        {exchangeRateLoader ? (
          <p className="text-[#E33A24]">{t('calculating_exchange_rate')}</p>
        ) : exchangeRateInfo?.success ? (
          <>
            <p className="text-xs text-white md:text-sm">
              {t('you_will_receive_approximately')}
            </p>
            <p className="text-base font-bold text-[#E33A24] md:text-lg">
      ...
      <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Input
                ...
                className="relative block h-[46px] w-full appearance-none rounded-[5px] border border-[#E33A2480] bg-transparent px-3 py-3 text-white shadow-none placeholder:text-xs placeholder:text-[#FFFFFF66] focus:border-[#E33A2480] focus:bg-[#172F31CC] focus:ring-0 focus:ring-transparent focus:outline-none sm:text-sm md:placeholder:text-sm lg:h-[55px]"
        ...
        <div className="space-y-2">
          ...
          >
            {renderAmountLabel()}
            {currency && (
              <span className="text-xs text-[#E33A24]">
                {' '}
                (
                {minimumAmountLoader
                  ? t('loading')
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <button
              ...
              className="template10-filled-button-hover mb-1 flex w-full cursor-pointer items-center justify-center rounded-[4px] bg-[#E33A24] px-8 pt-4 pb-4 text-[14px] font-semibold text-white transition-all duration-200 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 md:w-auto"
            >
              <span className="text-container">
                <span className="text">{renderButtonContent()}</span>
              </span>
            </button>
          </div>
