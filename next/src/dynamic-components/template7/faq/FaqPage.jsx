  ...
  const Section = ({ items }) => (
    <div className="space-y-4">
      {items.map((item, idx) => (
        <div
          key={idx}
          className="rounded-[3px] border border-[#3E1D88] transition-colors duration-200 hover:border-[#641AB9]"
        >
          ...
          <div className="flex flex-wrap gap-2">
            {tabs.map((tab) => (
              <button
                ...
                className={`mobile-tab-button rounded-[3px] border border-[#3E1D88] px-3 py-2 text-sm transition-colors duration-200 focus:ring-0 focus:outline-none focus-visible:outline-none sm:px-4 sm:py-3 sm:text-base ${
                  activeTab === tab.key
