...
export default function DepositBankTab() {
  ...
  const PaymentMethodCard = ({ method, methodType }) => (
    <button
      ...
      className="flex h-[64px] w-full cursor-pointer items-center justify-center rounded-[5px] border border-[#3E1D88] bg-[#3E1D88] transition-all duration-300 hover:border hover:border-[#3E1D88] hover:bg-[#3E1D88] hover:shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]"
    >
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-[#7351FF] md:text-sm">
            ({approvalType})
          </span>
        </div>
        ...
        {!isLast && (
          <div className="py-4">
            <div className="border-opacity-50 w-full border-b border-[#3E1D88]" />
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="space-y-6 overflow-hidden">
      <div className="mt-4 rounded-[5px] border border-[#3E1D88] bg-[#331369] p-3 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)] md:p-4 lg:p-6">
      ...
      <div className="mt-8">
        <h3 className="font-bring-race mb-4 text-[20px] text-white md:text-[22px]">
          {t('deposit_history')}
        </h3>
        <DepositHistory />
      </div>
