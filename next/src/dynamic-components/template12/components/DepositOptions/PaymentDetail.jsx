...
const DropdownOption = memo(({ option, onSelect, field }) => (
  <div
    key={option}
    className="cursor-pointer px-3 py-2 text-xs text-white transition-colors duration-150 hover:bg-[#D3AF37] md:text-sm"
    onClick={() => {
  ...
  const accountDetails = useMemo(() => {
    ...
    if (selectedMethod?.accountData) {
      return {
        ...
        qrCode: selectedMethod.accountData.qr_code_path || null,
  ...
  return (
    <div className="space-y-6 text-white">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2 sm:gap-4">
        <button
          onClick={handleBack}
          className="rounded border border-[#FFFFFF66] text-white transition-colors hover:text-gray-300 focus:ring-2 focus:ring-[#D3AF37] focus:outline-none"
        >
          <Image
            src="https://d3emlo5tm9es2f.cloudfront.net/next/icons/back-icon.svg"
            alt="Back"
            width={20}
            height={20}
        ...
        <div className="flex items-center gap-2 rounded bg-[#1a1a1a] px-3 py-2 text-xs sm:text-sm">
          <Image
      ...
      <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                render={({ field }) => (
                  <div className="flex w-full flex-col gap-1">
                    <div className="relative w-full" ref={sendingDropdownRef}>
                      <div
                        className={`relative flex h-[46px] w-full cursor-pointer appearance-none items-center justify-between rounded-[5px] border px-3 py-3 text-white shadow-none focus:ring-0 focus:ring-transparent focus:outline-none sm:text-sm lg:h-[55px] ${
                          errors.sendingOption
                            ...
                            : 'border-[#D3AF3780] bg-transparent focus:border-[#D3AF37]'
                        }`}
                      ...
                      {isSendingOpen && (
                        <div className="absolute z-20 mt-1 max-h-56 w-full overflow-auto rounded-[5px] border border-[#D3AF3780] bg-[#1a1a1a] shadow-xl">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    ...
                    className="relative block h-[46px] w-full appearance-none rounded-[5px] border border-[#D3AF3780] bg-transparent px-3 py-3 text-white shadow-none placeholder:text-xs placeholder:text-[#FFFFFF66] placeholder:capitalize focus:border-[#D3AF37] focus:ring-0 focus:ring-transparent focus:outline-none sm:text-sm md:placeholder:text-sm lg:h-[55px]"
              ...
              <Controller
                ...
                render={({ field: { onChange } }) => (
                  <div className="flex w-full flex-col gap-1">
                    <div
                      className={`flex w-full flex-wrap items-center gap-3 rounded-[5px] border px-3 py-2 shadow-none transition-colors focus-within:ring-0 focus-within:ring-transparent focus-within:outline-none ${
                        errors.receiptFile
                          ...
                          : 'border-[#D3AF3780] bg-transparent focus-within:border-[#D3AF37] hover:border-[#D3AF37]'
                      }`}
                      onDragOver={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                    ...
                    >
                      <div
                        className={`btn-style901 rounded-md bg-[#E8D25E] px-4 py-2 text-xs font-semibold text-black [box-shadow:inset_0_-3px_0_#876800] transition-opacity focus-within:ring-2 focus-within:ring-[#D3AF37] focus-within:outline-none active:scale-95 ${
                          isFileDialogOpening
            ...
            <div className="mt-6 space-y-3">
                    ...
                    >
                      ...
                      <span
                        ...
                        className={`grid h-5 w-5 flex-shrink-0 place-items-center rounded-[3px] border-2 transition-all duration-200 ${
                          field.value
                            ? 'border-[#00BA00] bg-[#00BA00]'
                            : 'border-[#D3AF3780] bg-transparent hover:border-[#D3AF37]'
                        }`}
                      >
                        <svg
                          viewBox="0 0 24 24"
              ...
              <div className="mt-6 flex flex-row justify-center gap-3 sm:gap-4">
                <button
                  type="button"
                  onClick={handleBack}
                  className="btn-style503 btn-style901 inline-flex items-center justify-center rounded-[10px] border-2 border-[#FFFFFF66] bg-transparent px-6 py-3 text-[14px] font-semibold text-white transition-all duration-200 hover:bg-[#D3AF37]/10 hover:[box-shadow:0_0_10px_0_#876800_inset,0_0_20px_2px_#876800] hover:outline hover:outline-2 hover:outline-[#876800] active:scale-95"
                >
                  {t('back')}
                </button>
                <button
                  type="submit"
                  disabled={!watchedValues.termsAccepted || isSubmitting}
                  className="flex cursor-pointer items-center justify-center rounded-[10px] bg-[#E8D25E] px-6 pt-3 pb-4 text-[14px] font-semibold text-black [box-shadow:inset_0_-6px_0_#876800] transition-all duration-200 hover:[box-shadow:0_0_10px_0_#876800_inset,0_0_20px_2px_#876800] hover:outline hover:outline-2 hover:outline-[#876800] active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                  data-hover={isSubmitting ? t('processing') : t('submit')}
                >
        ...
        <div className="order-1 h-max rounded-[16px] border border-[#FFFFFF66] bg-black p-5 md:sticky md:top-4 md:order-2">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs md:text-sm">
                {t('status')}:{' '}
                <span className="text-[#00BA00]">{t('status_online')}</span>
              </span>
              <Image
                src="https://d3emlo5tm9es2f.cloudfront.net/next/icons/green-check.svg"
                alt="online"
          ...
          <div className="grid grid-cols-1">
            ...
            {/* Last two fields with QR code layout */}
            <div className="flex flex-col gap-4 md:flex-row">
              {/* Labels column - hidden on mobile, only show if QR code exists */}
              {accountDetails?.qrCode && (
                <div className="hidden flex-col md:flex md:w-1/3">
                  <span className="mb-6 block text-[14px] font-bold text-white md:pt-3">
                    {t('commission')}
                  </span>
                  <span className="mb-6 block text-[14px] font-bold text-white md:mt-10 md:mb-0">
                    {t('subsidi')}
                  </span>
                </div>
              )}
              {/* Inputs and QR code shared container */}
              <div
                className={`flex flex-col gap-4 md:flex-row ${
                  accountDetails?.qrCode ? 'md:w-2/3 md:flex-1' : 'w-full'
                }`}
              >
                {/* Inputs column */}
                <div
                  className={`w-full ${accountDetails?.qrCode ? 'md:flex-1' : ''}`}
                >
                  {/* Commission field */}
                  <div className="mb-6 flex flex-col md:block">
                    <span className="mb-2 block text-[14px] font-bold text-white md:hidden">
                      {t('commission')}
                    </span>
                ...
                {/* QR Code column - only show if QR code exists */}
                {accountDetails?.qrCode && (
                  <div className="flex w-full flex-col items-center justify-center md:w-auto md:shrink-0">
                    <button
                      type="button"
                    ...
                    <p className="mt-2 cursor-pointer text-center text-xs text-white hover:opacity-80">
                      {t('click_to_view')}
                    </p>
                  </div>
                )}
          ...
          <div className="mt-4 text-[12px]">
            <span className="font-semibold text-[#D3AF37]">{t('note')}:</span>
      ...
      {isQRPreviewOpen && accountDetails?.qrCode && (
        <div
        ...
        >
          ...
          {/* Modal content */}
          <div
            className="relative z-10 flex flex-col items-center justify-center rounded-lg border border-[#FFFFFF66] bg-black p-4"
          ...
          >
            ...
            <button
              type="button"
              onClick={() => setIsQRPreviewOpen(false)}
              className="absolute top-2 right-2 flex h-8 w-8 items-center justify-center rounded-full bg-[#E8D25E] text-black transition-opacity hover:opacity-80"
