...
export default function CryptoSelect({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`flex-1 rounded-[4px] border px-4 py-2 text-[12px] font-bold whitespace-nowrap transition-all duration-300 md:flex-none md:px-6 md:py-2.5 md:text-[14px] ${
                    category === cat.key
                      ? ' bg-[#CBBC91] text-[#000000]'
                      : 'border-[#CBBC91] bg-[#0F50451A] text-[#CBBC91]'
                  }`}
                >
                  {cat.label}
                </button>
      ...
      <div className="rounded-[5px] border border-[#CBBC9180] bg-transparent p-4 md:p-5">
        <div className="flex flex-col items-start gap-2 pb-2 sm:flex-row sm:items-center sm:gap-4 md:pb-3">
          ...
          {showSearch && (
            <input
              ...
              className="relative block h-[46px] w-full appearance-none rounded-[5px] border border-[#CBBC9133] bg-[#0F50451A] px-3 py-3 text-white shadow-none placeholder:text-xs placeholder:text-[#FFFFFF66] focus:border-[#CBBC91] focus:ring-0 focus:ring-transparent focus:outline-none sm:w-auto sm:flex-1 sm:text-sm md:px-4 md:text-[14px] md:placeholder:text-sm lg:h-[55px]"
            />
          )}
        </div>
        <div
                ...
                <button
                  ...
                  className={`group flex min-h-[80px] w-full items-center justify-between gap-3 rounded-[5px] border-2 px-4 py-4 text-[#cfd8df] transition-all duration-300 sm:px-4 ${
                    selectedCurrency === c.code
                      ? 'border-[#CBBC9133] bg-[#0C0C0C]'
                      : 'border-[#CBBC9133] bg-transparent'
                  } hover:border-[#CBBC91] focus:border-[#CBBC91] focus:shadow-[inset_0_4.88px_29.278px_0_#0F5045,0_0_20px_rgba(15,80,69,0.6)]`}
                >
