...
const CryptoPaymentDetails = ({ paymentData, onClose }) => {
  ...
  const renderAmountDisplay = () => (
    <div className="rounded-[3px] border border-[#3E1D88] bg-[#3E1D88] px-2 py-4 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
      <div className="text-center text-base font-bold md:text-lg">
        <span className="text-[#EC79F2]">{displayAmount.amount}</span>{' '}
        <span className="text-[#EC79F2]">{displayAmount.currency}</span>
      </div>
    </div>
  );
  ...
  const renderCryptoAmountDisplay = () => (
    <div className="rounded-[3px] border border-[#3E1D88] bg-[#3E1D88] px-2 py-4 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
      <div className="text-center text-base font-bold md:text-lg">
        <span className="text-[#EC79F2]">{cryptoAmount.amount}</span>{' '}
        <span className="text-[#EC79F2]">{cryptoAmount.currency}</span>
      </div>
    </div>
  );
  ...
  const renderQRCode = () => {
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <div className="mt-3 text-xs tracking-wide text-[#EC79F2] uppercase">
          {t('scan_to_pay')}
        </div>
      </div>
    );
  ...
  const renderAddressSection = () => (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
      <div className="flex h-[46px] w-full items-center rounded-[3px] border border-[#3E1D88] bg-transparent px-3 text-[12px] text-white md:text-[14px]">
      ...
      <button
        className="fancy-hover-effect-orange w-full rounded-[10px] bg-[#641AB9] px-5 py-3 text-sm font-bold text-white transition-all duration-200 active:scale-95 sm:w-[270px]"
        onClick={() => copyToClipboard(paymentData.pay_address)}
      >
  ...
  return (
    <div className="mt-4 rounded-[5px] border border-[#3E1D88] bg-transparent p-5">
      {/* Header */}
      <div className="mb-6 flex items-center justify-start gap-4">
        <button
          onClick={onClose}
          aria-label={t('back')}
          className="angled-button angled-button-pinks flex h-[45px] w-[125px] items-center justify-center rounded-[4px] text-[14px] font-extrabold text-white transition-all duration-300"
          data-hover={t('back')}
        >
      ...
      <div className="grid grid-cols-1 items-start gap-4 md:grid-cols-3">
        <div className="md:col-span-2">
          ...
          <div className="rounded-[8px] border border-[#3E1D88] bg-transparent px-2 py-2 text-white">
      ...
      {/* Instructions */}
      <div className="mt-4 rounded-[5px] border border-[#3E1D88] bg-[#3E1D88] p-3 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
        <div className="mt-4">
