...
import CryptoSelect from '@/dynamic-components/template14/components/CryptoSelect/CryptoSelect';
import CryptoWithdrawalDetails from '@/dynamic-components/template14/components/CryptoWithdrawalDetails/CryptoWithdrawalDetails';
...
export default function WithdrawalCryptoTab() {
  ...
  const verifyButtonClassName = useMemo(() => {
    ...
    return `${baseClasses} bg-[#641AB9] text-white font-semibold   hover:shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]`;
  }, [addressVerificationStatus]);
  ...
  const renderExchangeRateDisplay = () => {
    ...
    return (
      <div className="mt-2 rounded-[3px] border border-[#3E1D88] bg-[#3E1D88] p-3 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
        {exchangeRateLoader ? (
          <p className="text-[#7351FF]">{t('calculating_exchange_rate')}</p>
        ) : exchangeRateInfo?.success ? (
          <>
            <p className="text-xs text-white md:text-sm">
              {t('you_will_receive_approximately')}
            </p>
            <p className="text-base font-bold text-[#7351FF] md:text-lg">
      ...
      <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="space-y-2">
          ...
          >
            {renderAmountLabel()}
            {currency && (
              <span className="text-xs text-[#7351FF]">
                {' '}
                (
                {minimumAmountLoader
                  ? t('loading')
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <button
              ...
              className="angled-button angled-button-pinks flex h-[48px] min-w-[160px] cursor-pointer items-center justify-center rounded-[10px] px-6 pt-3 pb-3 text-[14px] font-semibold text-white transition-all duration-200 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 md:w-auto"
            >
