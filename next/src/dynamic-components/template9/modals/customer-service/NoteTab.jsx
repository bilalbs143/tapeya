...
export default function NoteTab({ openMessageId }) {
  ...
  return (
    <div className="flex h-full flex-col">
      <h3 className="font-cravend text-center text-[20px] sm:text-left sm:text-[35px] md:text-[35px]">
        {t('notes')}
      </h3>
                ...
                <tbody>
                  {notesData.length === 0 ? (
                    ...
                    notesData.map((item, index) => (
                      <tr
                        key={item.id || index}
                        className="cursor-pointer border-t border-b border-[#DBB42C4D] transition-all duration-300 hover:border-[#DBB42C4D] hover:bg-[#12001F] hover:shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]"
