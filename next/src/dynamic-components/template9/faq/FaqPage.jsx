  ...
  const Section = ({ items }) => (
    <div className="space-y-4">
      {items.map((item, idx) => (
        <div
          key={idx}
          className="rounded-[3px] border border-[#DBB42C4D] transition-colors duration-200 hover:border-[#DBB42C4D]"
        >
          ...
          <div className="flex flex-wrap gap-2">
            {tabs.map((tab) => (
              <button
                ...
                className={`mobile-tab-button rounded-[3px] border border-[#DBB42C4D] px-3 py-2 text-sm transition-colors duration-200 focus:ring-0 focus:outline-none focus-visible:outline-none sm:px-4 sm:py-3 sm:text-base ${
                  activeTab === tab.key
                    ? 'bg-[#9D4EDD] text-white'
                    : 'bg-[#12001F] text-white'
                }`}
              >
                {tab.label}
              </button>
