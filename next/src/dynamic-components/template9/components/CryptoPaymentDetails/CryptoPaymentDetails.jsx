...
const CryptoPaymentDetails = ({ paymentData, onClose }) => {
  ...
  const renderAmountDisplay = () => (
    <div className="rounded-[3px] border border-[#DBB42C4D] bg-[#1D0032] px-2 py-4 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
      <div className="text-center text-base font-bold md:text-[25px]">
        <span className="text-[#DBB42C]">{displayAmount.amount}</span>{' '}
        <span className="text-[#DBB42C]">{displayAmount.currency}</span>
      </div>
    </div>
  );
  ...
  const renderCryptoAmountDisplay = () => (
    <div className="rounded-[3px] border border-[#DBB42C4D] bg-[#1D0032] px-2 py-4 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
      <div className="text-center text-base font-bold md:text-[25px]">
        <span className="text-[#DBB42C]">{cryptoAmount.amount}</span>{' '}
        <span className="text-[#DBB42C]">{cryptoAmount.currency}</span>
      </div>
    </div>
  );
  ...
  const renderQRCode = () => {
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <div className="mt-3 text-xs tracking-wide text-[#DBB42C] uppercase">
          {t('scan_to_pay')}
        </div>
      </div>
    );
  ...
  const renderAddressSection = () => (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
      <div className="flex h-[46px] w-full items-center rounded-[3px] border border-[#DBB42C4D] bg-transparent px-3 text-[12px] text-white md:text-[14px]">
      ...
      <button
        className="template9-filled-button-hover w-full rounded-[4px] bg-[#9D4EDD] px-5 py-3 text-sm font-bold text-white transition-all duration-200 active:scale-95 sm:w-[270px]"
        onClick={() => copyToClipboard(paymentData.pay_address)}
      >
  ...
  return (
    <div className="mt-4 rounded-[5px] border border-[#DBB42C4D] bg-[#12001F] p-5">
      {/* Header */}
      <div className="mb-6 flex items-center justify-start gap-4">
        <button
          onClick={onClose}
          aria-label={t('back')}
          className="template9-filled-button-hover flex items-center justify-center rounded-[4px] bg-[#9D4EDD] px-12 py-4 font-normal text-white transition-all duration-300"
        >
          <span className="text-container">
            <span className="text">Back</span>
          </span>
        </button>
        <span className="text-[15px] font-bold text-white md:ml-2 md:text-[16px]">
          {t('deposit_details')}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 items-start gap-4 md:grid-cols-3">
        <div className="md:col-span-2">
          ...
          <div className="rounded-[8px] border border-[#DBB42C4D] bg-transparent px-2 py-2 text-white">
