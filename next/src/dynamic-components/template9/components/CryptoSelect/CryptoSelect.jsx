...
export default function CryptoSelect({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="mt-6 rounded-[5px] border border-[#DBB42C4D] bg-[#12001F] p-4 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)] md:p-5">
        {showCategories && (
          <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-1 md:gap-2">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`flex-1 rounded-[4px] px-3 py-2.5 text-[12px] font-bold whitespace-nowrap transition-all duration-300 md:flex-none md:border-0 md:px-6 md:py-3 md:text-[14px] ${
                      category === cat.key
                        ? 'bg-[#9D4EDD] text-white'
                        : 'bg-[#0A1414] text-white'
                    }`}
                  >
                    {cat.label}
                  </button>
        ...
        <div className="flex flex-col items-start gap-2 pb-2 sm:flex-row sm:items-center sm:gap-4 md:pb-3">
          ...
          {showSearch && (
            <input
              ...
              className="relative block h-[46px] w-full appearance-none rounded-[5px] border border-[#DBB42C4D] bg-transparent px-3 py-3 text-white shadow-none placeholder:text-xs placeholder:text-[#FFFFFF66] focus:border-[#DBB42C] focus:bg-[#1D0032] focus:ring-0 focus:ring-transparent focus:outline-none sm:w-auto sm:flex-1 sm:text-sm md:px-4 md:text-[14px] md:placeholder:text-sm lg:h-[55px]"
            />
          )}
        </div>
                ...
                <button
                  ...
                  className={`group flex min-h-[80px] w-full items-center justify-between gap-3 rounded-[5px] border-2 px-4 py-4 text-[#cfd8df] transition-all duration-300 sm:px-4 ${
                    selectedCurrency === c.code
                      ? 'border-[#DBB42C] bg-[#060D0D]'
                      : 'border-[#DBB42C4D] bg-[#060D0D]'
                  } focus:border-[#DBB42C]`}
                >
