...
import CryptoSelect from '@/dynamic-components/template6/components/CryptoSelect/CryptoSelect';
import CryptoWithdrawalDetails from '@/dynamic-components/template6/components/CryptoWithdrawalDetails/CryptoWithdrawalDetails';
...
export default function WithdrawalCryptoTab() {
  ...
  const verifyButtonClassName = useMemo(() => {
    ...
    return `${baseClasses} bg-[#D61324] text-white font-semibold   hover:shadow-[inset_0_4.402px_26.412px_0_#E70011]`;
  }, [addressVerificationStatus]);
  ...
  const renderExchangeRateDisplay = () => {
    ...
    return (
      <div className="mt-2 rounded-lg bg-[#101010] p-3">
        {exchangeRateLoader ? (
          <p className="text-[#D61324]">{t('calculating_exchange_rate')}</p>
        ) : exchangeRateInfo?.success ? (
          <>
            <p className="text-xs text-white md:text-sm">
              {t('you_will_receive_approximately')}
            </p>
            <p className="text-base font-bold text-[#D61324] md:text-lg">
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-[15px] font-bold text-white md:text-[19px]">
      ...
      <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="space-y-2">
          ...
          >
            {renderAmountLabel()}
            {currency && (
              <span className="text-xs text-[#D61324]">
                {' '}
                (
                {minimumAmountLoader
                  ? t('loading')
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <button
              ...
              className="fancy-hover-effect-orange flex w-full cursor-pointer items-center justify-center rounded-[10px] bg-[#D61324] px-6 pt-3 pb-3 text-[14px] font-semibold text-white transition-all duration-200 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 md:w-auto"
            >
