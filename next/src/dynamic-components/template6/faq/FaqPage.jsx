...
function FaqPage() {
  ...
  const Section = ({ items }) => (
    <div className="space-y-4">
      {items.map((item, idx) => (
        <div
          key={idx}
          className="rounded-lg border border-[#272727] transition-colors duration-200 hover:border-[#FB63214D]"
        >
          <div className="mobile-faq-question bg-[#FFFFFF] bg-clip-text px-4 py-3 font-semibold text-white">
            {item.q}
          </div>
          <div className="mobile-faq-answer px-4 pb-4 text-[10px] text-[#CCCCCC] md:text-[16px]">
            {item.a}
          </div>
        </div>
      ))}
  ...
  return (
    <div className="min-h-screen text-white">
      <div className="container px-2 py-8 md:mx-auto">
        <h1 className="mobile-title mb-2 mb-8 bg-[#FFFFFF] bg-clip-text text-[20px] font-semibold text-transparent md:text-[30px]">
          ...
          <div className="flex flex-wrap gap-2">
            {tabs.map((tab) => (
              <button
                ...
                className={`mobile-tab-button rounded-lg border border-[#272727] px-3 py-2 text-sm transition-colors duration-200 focus:ring-0 focus:outline-none focus-visible:outline-none sm:px-4 sm:py-3 sm:text-base ${
                  activeTab === tab.key
                    ? 'border-[#FB63214D] bg-[#D61324] font-semibold text-white'
                    : 'bg-transparent text-white hover:border-[#FB63214D] hover:bg-[#D61324]/20'
                }`}
              >
                {tab.label}
              </button>
        ...
        <div
          ...
          style={{ borderColor: '#FB63214D' }}
        >
          {(() => {
            const items = tabContent[activeTab] || [];
            if (!items.length) {
              return (
                <div className="text-sm text-white">{t('no_data_found')}</div>
              );
            }
