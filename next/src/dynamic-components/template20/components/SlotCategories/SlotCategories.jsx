    ...
    <section className="md:py-3">
      <div className="mx-auto px-4 md:px-4">
        ...
        {accumulatedGames.length > 0 ? (
          <div>
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-8 md:gap-4">
              {accumulatedGames.map((game, index) => (
                <GameCard
                  key={`${game.id}-${index}`}
                  game={game}
                  index={index}
                  className="mx-auto h-[105px] w-[115px] md:h-[196px] md:w-full md:max-w-[220px]"
                  imageClassName="h-full w-full md:h-full"
                />
              ))}
            </div>
          </div>
        ...
        <div className="mt-6 flex flex-col items-center justify-center gap-4">
          {hasMore && (
            <button
              ...
              className="inline-flex transform cursor-pointer items-center justify-center rounded-[5px] bg-[#D00000] px-8 py-3 font-semibold text-[#ffffff] transition-all hover:scale-105 hover:bg-[#D000004D] active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          {accumulatedGames.length > 60 && (
            <button
              onClick={scrollToTop}
              className="inline-flex transform cursor-pointer items-center justify-center rounded-[10px] bg-[#D00000] px-6 py-3 font-semibold text-[#ffffff] transition-all hover:scale-105 hover:bg-[#D000004D] active:scale-95"
            >
              {t('back_to_top')}
            </button>
          )}
        ...
        {currentPage === 1 &&
          ...
          <div className="py-16">
            <div className="flex flex-col items-center space-y-4">
              <CommonLoader size="xl" border="border-[#FFDAB9]" />
