...
export default function CryptoSelect({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
            ...
            <div className="flex flex-wrap gap-1 md:gap-3">
              {categories.map((cat) => (
                <button
                  ...
                  className={`flex-1 rounded-[4px] border px-4 py-2 text-[12px] font-bold whitespace-nowrap transition-all duration-300 md:flex-none md:px-6 md:py-2.5 md:text-[14px] ${category === cat.key
                      ? 'border-[#5858584D] bg-[#D00000] text-[white]'
                      : 'border-[#5858584D] bg-transparent text-[white]'
                  }`}
                >
                  {cat.label}
                </button>
      ...
      <div className="rounded-[5px] border border-[#5858584D] bg-transparent p-4 md:p-5">
          ...
          <div className="xs:grid-cols-2 grid grid-cols-1 gap-2 py-3 sm:grid-cols-2 sm:gap-3 md:grid-cols-3 md:gap-4 md:py-4 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-5">
            {loadingCurrencies ? (
              <div className="col-span-full flex items-center justify-center py-8">
                <CommonLoader size="lg" border="border-[#FFDAB9]" />
              </div>
              ...
              visibleCoins.map((c) => (
                <button
                  ...
                  className={`group flex min-h-[80px] w-full items-center justify-between gap-3 rounded-[5px] border-2 px-4 py-4 text-[#cfd8df] transition-all duration-300 sm:px-4 ${selectedCurrency === c.code
                      ? 'border-[#CBBC9133] bg-[#0F50451A]'
                      : 'border-[#5858584D] bg-transparent'
                  } hover:border-[#D00000] focus:border-[#D00000] focus:shadow-[inset_0_4px_24px_0_rgba(208,0,0,0.60)]`}
                >
                  ...
                  {c.icon ? (
                    ...
                    <div
                      ...
                      style={{ backgroundColor: c.color || '#D00000' }}
                    >
