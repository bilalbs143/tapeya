...
const CryptoPaymentDetails = ({ paymentData, onClose }) => {
  ...
  const renderAmountDisplay = () => (
    <div className="rounded-[10px] border border-[#03c72c4d] bg-black px-4 py-4">
      <div className="text-center text-base font-bold md:text-lg">
        <span className="text-[#5AB25A]">{displayAmount.amount}</span>{' '}
        <span className="text-[#5AB25A]">{displayAmount.currency}</span>
      </div>
    </div>
  );
  ...
  const renderCryptoAmountDisplay = () => (
    <div className="rounded-[10px] border border-[#03c72c4d] bg-black px-4 py-4">
      <div className="text-center text-base font-bold md:text-lg">
        <span className="text-[#5AB25A]">{cryptoAmount.amount}</span>{' '}
        <span className="text-[#5AB25A]">{cryptoAmount.currency}</span>
      </div>
    </div>
  );
  ...
  const renderQRCode = () => {
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <div className="mt-3 text-xs tracking-wide text-[#5AB25A] uppercase">
          {t('scan_to_pay')}
        </div>
      </div>
    );
  ...
  const renderAddressSection = () => (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
      <div className="flex h-[46px] w-full items-center rounded-[6px] border border-[#03c72c4d] bg-[#ffffff17] px-3 text-[12px] text-white md:text-[14px]">
        <div className="flex flex-1 items-center gap-2">
          ...
          <button
            ...
            aria-label={t('copy')}
            className="filled-hover-effect btn-style901 shrink-0 transition-opacity hover:opacity-80"
          >
            <span>
              <Image
      ...
      <button
        className="filled-hover-effect btn-style901 w-full rounded-[10px] bg-[#5AB25A] px-5 py-3 text-sm font-bold text-black transition-all duration-200 active:scale-95 sm:w-[270px]"
        onClick={() => copyToClipboard(paymentData.pay_address)}
      >
        <span>{t('copy')}</span>
      </button>
    ...
    <div className="rounded-[16px] border border-[#FFFFFF66] bg-transparent p-5">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          onClick={onClose}
          aria-label={t('close')}
          className="group flex h-8 w-8 flex-shrink-0 cursor-pointer items-center justify-center rounded-md bg-[#55BC55] text-black transition-all duration-300"
        >
          <svg
            width="16"
            height="16"
        ...
        <div className="md:col-span-2">
          ...
          <div className="rounded-[8px] border border-[#FFFFFF66] bg-transparent px-6 py-8 text-white">
            <div className="flex flex-col gap-3 md:flex-row md:items-center">
              ...
              <div className="text-center text-2xl font-bold text-[#55BC55] select-none md:hidden">
                =
              </div>
              <div className="hidden px-2 text-center text-2xl font-bold text-[#55BC55] select-none md:block">
                =
              </div>
      ...
      <div className="mt-6">
        ...
        <div className="rounded-[8px] border border-[#03c72c4d] bg-[#0A1818] px-5 py-4 text-white">
          {renderInstructions()}
        </div>
      </div>
    </div>
