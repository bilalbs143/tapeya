...
export default function CustomerService({
  ...
  return (
    <div className="customer-service-modal relative mx-auto flex h-[80vh] w-full max-w-[1479px] flex-col overflow-hidden bg-[#000000] text-white shadow-xl">
      <div className="flex min-h-0 flex-1 flex-col space-y-4 p-4 md:space-y-6 md:p-6 lg:p-8">
          ...
          <div className="p-[1px]">
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`min-w-[200px] rounded-[4px] px-6 py-3.5 text-[14px] font-semibold transition-all duration-200 ${
                      activeTab === tab.key
                        ? 'border border-[#D00000] bg-[#D000001A] text-[#FFFFFF]'
                        : 'border border-[#5858584D] text-[#CCCCCC] hover:border-[#D00000]'
                    } `}
                  >
                    {tab.label}
                  </button>
            ...
            <div className="mt-3 w-full rounded-[6px] p-[4px] md:hidden">
              <button
                ...
                className="relative flex w-full items-center justify-between border-0 px-6 py-2 text-[13px] font-normal tracking-wide text-white/70 uppercase transition-all duration-300 before:absolute before:inset-0 before:rounded-[3px] before:border before:border-[#FFDAB9] before:bg-[#D000001A]"
              >
        ...
        {/* Content Box */}
        <div className="scrollbar-hide mt-3 min-h-0 flex-1 overflow-y-auto rounded-[7px] border border-[#5858584D] p-3 md:p-4 lg:p-4">
          {/*  Profile */}
          {activeTab === 'profile' && (
