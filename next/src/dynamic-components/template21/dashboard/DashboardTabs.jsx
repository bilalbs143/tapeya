...
import AnnouncementsPage from '@/dynamic-components/template21/announcements/AnnouncementsPage';
import LazyImage from '@/dynamic-components/template21/components/LazyImage/LazyImage';
import BettingManagementPage from '@/dynamic-components/template21/dashboard/BettingManagementPage';
import DepositPage from '@/dynamic-components/template21/dashboard/DepositPage';
import ProfilePage from '@/dynamic-components/template21/dashboard/ProfilePage';
import ReferralsPage from '@/dynamic-components/template21/dashboard/ReferralsPage';
import WithdrawalPage from '@/dynamic-components/template21/dashboard/WithdrawalPage';
import T21_LiveCasino from '@/dynamic-components/template21/live-casino/LiveCasinoPage';
import InquiryTab from '@/dynamic-components/template21/modals/customer-service/InquiryTab';
import NoteTab from '@/dynamic-components/template21/modals/customer-service/NoteTab';
import CouponsTab from '@/dynamic-components/template21/modals/transaction/CouponsTab';
import ExchangeTab from '@/dynamic-components/template21/modals/transaction/ExchangeTab';
import PointsTab from '@/dynamic-components/template21/modals/transaction/PointsTab';
import PromotionsPage from '@/dynamic-components/template21/promotions/PromotionsPage';
...
// Tab button width: min-width in px (tabs can grow for longer labels)
const TAB_MIN_WIDTH_PX = 180;

// Categories – same list and icons as Categories component (template21)
const categories = [
...
function DashboardTabs() {
  ...
  const primaryTabs = useMemo(() => {
    ...
    const hiddenPrimaryKeys = ['fishing', 'event', 'promotions'];
    return categories
      .filter((category) => !hiddenPrimaryKeys.includes(category.key))
      .map((category) => ({
        key: category.key,
  ...
  const renderContent = () => {
    ...
    if (activePrimary === 'other') {
      return (
        <div className="scrollbar-hide min-h-[300px] overflow-y-auto">
          <T21_Slots categoryOverride="arcade" />
        </div>
      );
    }
    ...
    if (activePrimary === 'table') {
      return (
        <div className="scrollbar-hide min-h-[300px] overflow-y-auto">
          <T21_LiveCasino categoryOverride="table" />
        </div>
      );
    }
      ...
      >
        <div className="flex flex-wrap mt-auto">
          {primaryTabs.map((tab) => {
            ...
            // Same enabled categories as Categories component (template21)
            const enabledTabs = ['home', 'slots', 'casino', 'promotions', 'sports', 'other', 'table'];
            const isDisabled = !enabledTabs.includes(tab.key);
            
            const baseButtonClasses =
              'flex items-center justify-start gap-2 text-left text-[16px] font-medium transition-colors py-[10px] px-[15px] ml-[2px] max-w-full';
            const activeClasses =
              'bg-[#3e444c] text-white border border-[#1c1e22] border-b-0 rounded-t-[4px] mb-0';
            ...
            const commonProps = {
              ...
              style: tab.key === 'home' ? undefined : { minWidth: `${TAB_MIN_WIDTH_PX}px` },
            };
                ...
                <div className="flex min-w-0 flex-1 items-center justify-start gap-2">
                  {tab.icon && (
                    <LazyImage
                      ...
                      width={20}
                      height={20}
                      className={`h-4 w-4 sm:h-5 sm:w-5 object-contain [filter:brightness(0)_invert(1)] ${
          ...
          {secondaryTabs.map((tab) => {
            ...
            return (
              <button
                ...
                className="flex w-max max-w-full items-center justify-start gap-2 text-left text-white text-[14px] transition-all py-[10px] px-[15px] rounded-[4px] group ml-[2px] mb-[2px]"
                style={{
