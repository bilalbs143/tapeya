...
function HeroSection() {
  ...
  const mobileSlides = [
    'https://d3emlo5tm9es2f.cloudfront.net/next/backgrounds/home-top-banner-mob-17-1-up-2.png',
    'https://d3emlo5tm9es2f.cloudfront.net/next/backgrounds/home-top-banner-mob-17-2-up-2.png',
    'https://d3emlo5tm9es2f.cloudfront.net/next/backgrounds/home-top-banner-mob-17-3-up-2.png',
  ];
  ...
  return (
    ...
    >
      ...
      {/* Mobile Slider - auto height, original image size */}
      <div className="relative block md:hidden">
        {/* Spacer: reserves height from first image's natural aspect ratio */}
        <div className="relative w-full opacity-0 pointer-events-none select-none">
          <Image
            src={mobileSlides[0]}
            alt=""
            width={768}
        ...
        {mobileSlides.map((slide, index) => (
          <div
            key={index}
            className={`absolute inset-0 top-0 left-0 right-0 transition-opacity duration-700 ease-in-out ${
              index === currentSlide ? 'opacity-100' : 'opacity-0'
            }`}
          >
            <Image
        ...
        <div className="flex items-center gap-2">
          {desktopSlides.map((_, index) => (
            <button
              ...
              className={`h-1 w-8 cursor-pointer rounded-none transition-all duration-300 sm:h-1 sm:w-10 ${
                index === currentSlide
                  ? 'bg-white'
