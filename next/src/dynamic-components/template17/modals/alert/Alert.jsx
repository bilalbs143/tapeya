...
import PendingExchangeAlert from '@/dynamic-components/template17/components/AlertSections/PendingExchangeAlert';
import UnreadNotesAlert from '@/dynamic-components/template17/components/AlertSections/UnreadNotesAlert';
...
export default function Alert() {
  ...
  const handleViewInquiry = () => {
    ...
    // Then redirect to the dashboard customer inquiry page.
    // If we have a specific inquiry id, pass it via query param so the page can focus it.
    const inquiryId = modalProps?.data?.customerInquiry?.id;
    if (inquiryId) {
      router.push(`/dashboard/customer-inquiry?inquiryId=${inquiryId}`);
    } else {
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={handleCloseModal}
              aria-label={t('close')}
              className="group flex h-[33px] w-[33px] flex-shrink-0 cursor-pointer items-center justify-center rounded-md bg-[#E8D25E] text-black transition-all duration-300 sm:h-[44px] sm:w-[44px]"
            >
              <svg
                width="16"
                height="16"
            ...
            {isCustomerInquiryReplyAlert && (
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="flex w-full cursor-pointer items-center justify-center rounded-[10px] bg-[#E8D25E] px-4 pt-3 pb-4 text-sm font-semibold text-black [box-shadow:inset_0_-6px_0_#876800] active:scale-95"
