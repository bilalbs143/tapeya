...
export default function NoteTab({ openMessageId }) {
  ...
  return (
    <div className="flex h-full flex-col">
      <h3 className="font-bring-race mb-4 text-[25px] text-white md:text-[40px]">
        {t('notes')}
      </h3>
                ...
                <tbody>
                  {notesData.length === 0 ? (
                    ...
                    notesData.map((item, index) => (
                      <tr
                        key={item.id || index}
                        className="cursor-pointer border-t border-b border-[#2DFA1A4D] transition-all duration-300 hover:border-[#2DFA1A4D] hover:bg-[#0F1B1B] hover:shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]"
