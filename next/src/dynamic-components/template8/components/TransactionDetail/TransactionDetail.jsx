      ...
      <div className={`flex flex-col gap-2 ${className}`}>
        ...
        <div className="flex-1">
          <input
            ...
            value={displayValue}
            readOnly
            className={`h-[46px] w-full cursor-not-allowed rounded-[5px] border border-[#2DFA1A4D] bg-transparent px-3 py-3 text-[12px] text-[#FFFFFF80] placeholder:text-[#FFFFFF66] sm:text-sm md:placeholder:text-sm lg:h-[55px] ${contentClassName}`}
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-1">
        {/* Transaction Details */}
        <div className="rounded-[9px] border-[0.8px] border-[#2DFA1A4D] p-4">
          <div className="mb-4 flex items-center justify-between">
            <div className="font-bring-race text-[17px] font-semibold text-white">
              {title}
            </div>
            ...
            <button
              onClick={onClose}
              aria-label={t('close')}
              className="group flex h-[30px] w-[30px] flex-shrink-0 cursor-pointer items-center justify-center rounded-[5px] border border-[#2DFA1A4D] bg-[#2DFA1A] text-black transition-all duration-300 sm:h-[33px] sm:w-[33px]"
            >
              <svg
                width="16"
                height="16"
