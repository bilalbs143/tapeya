...
import GameCard from '@/dynamic-components/template8/components/GameCard/GameCard';
  ...
  return (
    <section className="">
      <div className="">
          ...
          <div>
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 md:gap-6 lg:grid-cols-5 xl:grid-cols-5">
              {visibleGames.map((game, index) => (
                <div key={`${game.id}-${index}`} className="flex flex-col">
                  <div className="flex-shrink-0">
                    <GameCard
                      game={game}
                      index={index}
                      className="h-full w-full"
                      imageClassName="h-[150px] md:h-[270px] w-full"
                  ...
                  {/* Game Name Container */}
                  <div className="mt-4 w-full flex-shrink-0 rounded-[3px] border border-[rgba(45,250,26,0.30)] bg-[#0A1414] pt-[14px] pr-[25px] pb-[13px] pl-[25px] shadow-[4px_5px_16px_0_rgba(0,0,0,0.25)_inset]">
        ...
        <div className="mt-6 flex flex-col items-center justify-center gap-4">
          {hasMore && (
            <button
              ...
              className="inline-flex transform cursor-pointer items-center justify-center rounded-[5px] px-8 py-3 font-semibold text-white transition-all hover:scale-105 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
              style={{
          ...
          {accumulatedGames.length > 60 && (
            <button
              onClick={scrollToTop}
              className="inline-flex transform cursor-pointer items-center justify-center rounded-[5px] px-6 py-3 font-semibold text-white transition-all hover:scale-105 active:scale-95"
              style={{
                border: '1px solid rgba(45, 250, 26, 0.30)',
                backgroundColor: 'transparent',
              }}
        ...
        {currentPage === 1 &&
          ...
          <div className="py-16">
            <div className="flex flex-col items-center space-y-4">
              <CommonLoader size="xl" border="border-[#2DFA1A]" />
        ...
        {/* Games Coming Soon - When provider is selected but has no games */}
        {visibleGames.length === 0 &&
          !allGamesLoader &&
          currentPage === 1 &&
          selectedProviderId &&
          allGamesData &&
          (allGamesData.meta?.total === 0 ||
            (allGamesData.data &&
              Array.isArray(allGamesData.data) &&
              allGamesData.data.length === 0)) && (
          <div className="py-16 text-center">
            <div className="mx-auto max-w-md">
              <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-white/10">
                <svg
                  className="h-10 w-10 text-[#2DFA1A]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <p className="mb-2 text-xl font-semibold text-white">
                {t('games_coming_soon') || 'Games are coming soon'}
              </p>
            </div>
          </div>
        )}

        {/* No Games Available - When no provider is selected */}
        {visibleGames.length === 0 &&
          !allGamesLoader &&
          currentPage === 1 &&
          !selectedProviderId && (
