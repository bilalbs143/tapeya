...
import CryptoSelect from '@/dynamic-components/template8/components/CryptoSelect/CryptoSelect';
import CryptoWithdrawalDetails from '@/dynamic-components/template8/components/CryptoWithdrawalDetails/CryptoWithdrawalDetails';
...
export default function WithdrawalCryptoTab() {
  ...
  const verifyButtonClassName = useMemo(() => {
    ...
    return `${baseClasses} bg-[#2DFA1A] text-black font-semibold   hover:shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]`;
  }, [addressVerificationStatus]);
  ...
  const renderExchangeRateDisplay = () => {
    ...
    return (
      <div className="mt-2 rounded-[3px] border border-[#2DFA1A4D] bg-[#060D0D] p-3 shadow-[inset_4px_5px_16px_0_rgba(0,0,0,0.25)]">
        {exchangeRateLoader ? (
          <p className="text-[#2DFA1A]">{t('calculating_exchange_rate')}</p>
        ) : exchangeRateInfo?.success ? (
          <>
            <p className="text-xs text-white md:text-sm">
              {t('you_will_receive_approximately')}
            </p>
            <p className="text-base font-bold text-[#2DFA1A] md:text-lg">
      ...
      <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="space-y-2">
          ...
          >
            {renderAmountLabel()}
            {currency && (
              <span className="text-xs text-[#2DFA1A]">
                {' '}
                (
                {minimumAmountLoader
                  ? t('loading')
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <button
              ...
              className="template8-filled-button-hover mb-1 flex w-full cursor-pointer items-center justify-center rounded-[4px] bg-[#2DFA1A] px-8 pt-4 pb-4 text-[14px] font-semibold text-white transition-all duration-200 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 md:w-auto"
            >
