...
export default function WalletDropdown({
  ...
  // Handle navigation to points page (redirect to coupons)
  ...
  return (
    <div className="relative" ref={walletRef}>
      <div
        className={`inline-flex ${buttonWidth} ${isMobile ? 'h-[50px]' : 'h-[54px]'} items-stretch ${isMobile ? 'rounded-[6px]' : 'rounded-[5px]'} border-2 ${isMobile ? 'border-[#00374A]' : 'border-[#03c72c4d]'}`}
      >
        <button
          onClick={() => setIsWalletOpen(!isWalletOpen)}
          className={`group flex w-full items-center justify-between ${buttonPadding} h-full cursor-pointer ${isMobile ? 'rounded-[6px]' : 'rounded-[5px]'} ${isMobile ? 'bg-[#00111A]' : 'bg-[#0A1818]'} text-white transition-all hover:shadow-[inset_0_0_6px_1px_#55BC55]`}
        >
          <span className="flex items-center gap-3">
            <Image
              src={`${baseUrl}/icons/coin-5.svg`}
              alt="Wallet"
              width={25}
              height={25}
            ...
            >
              {userLoader ? (
                <CommonLoader size="sm" border="border-[#20C5FE]" />
              ) : hasWalletData ? (
          ...
          <Image
            src={`${baseUrl}/icons/drop-5-blue-new.svg`}
            alt="Open wallet"
            width={15}
            height={15}
      ...
      {isWalletOpen && (
        <div
          className={`absolute ${isMobile ? 'right-0 left-0' : 'left-0'} ${isMobile ? 'w-full' : dropdownWidth} z-[1000] mt-2`}
        >
          {isMobile ? (
                  ...
                  <div className="overflow-hidden rounded-[7px] bg-[#001724]">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`font-medium text-white group-hover:text-[#20C5FE] ${textSize}`}
                        >
                          {userLoader ? (
            ...
            <div className="rounded-[0px] bg-[#20C5FE] p-[1px] shadow-xl">
              <div className="rounded-[0px] bg-[#000304] p-[10px]">
                <div className="rounded-[5px] bg-[#20C5FE] p-[1px]">
                  <div className="overflow-hidden rounded-[5px] bg-[#000304]">
                    {/* Row: Game Wallet */}
                    <button
                      type="button"
                      onClick={handleGameWalletClick}
                      className={`group flex w-full items-center justify-between ${dropdownPadding} border-b border-[#20C5FE] bg-transparent text-[20px] transition-all hover:bg-[#55BC55]`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={handlePointsClick}
                      className={`group flex w-full items-center justify-between ${dropdownPadding} border-b border-[#20C5FE] transition-all hover:border-[#55BC55] hover:bg-[#55BC55]`}
                    >
                      <div className="flex items-center gap-2 text-white group-hover:text-black">
                        <span className={textSize}>{t('points')}</span>
                      </div>
                    ...
                    <button
                      type="button"
                      onClick={handleCouponsClick}
                      className={`group flex w-full items-center justify-between ${dropdownPadding} bg-transparent transition-all hover:border-[#55BC55] hover:bg-[#55BC55]`}
                      style={{ borderBottom: '1px solid #20C5FE' }}
                    >
                      <div className="flex items-center gap-2 text-white group-hover:text-black">
                        <span className={textSize}>{t('coupons')}</span>
                      </div>
