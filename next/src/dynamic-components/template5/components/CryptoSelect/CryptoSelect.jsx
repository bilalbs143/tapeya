...
export default function CryptoSelect({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      {showAmount && (
        <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-4">
          ...
          <input
            ...
            className="relative block h-[46px] w-full appearance-none rounded-[5px] border border-[#03c72c4d] bg-transparent px-3 py-3 text-white shadow-none placeholder:text-xs placeholder:text-[#FFFFFF66] focus:border-[#5AB25A] focus:ring-0 focus:ring-transparent focus:outline-none sm:w-auto sm:flex-1 sm:text-sm md:px-4 md:text-[14px] md:placeholder:text-sm lg:h-[55px]"
          />
        </div>
      )}

      {showCategories && (
        <div className="mt-10 mb-[10px] rounded-[5px] border border-[#00374A] p-[1px] md:mt-5">
            ...
            <div className="flex flex-wrap gap-1 md:gap-2">
              {categories.map((t) => (
                <button
                  ...
                  className={`flex-1 rounded-[5px] border border-[#00374A] px-3 py-2 text-[12px] font-bold whitespace-nowrap transition-all duration-300 md:flex-none md:border-0 md:px-6 md:py-3 md:text-[14px] ${
                    category === t.key
                      ? 'bg-[#20C5FE] text-white'
                      : 'bg-[#00111A] text-white'
                  }`}
                >
                  {t.label}
                </button>
      ...
      <div className="rounded-[5px] border border-[#00374A] bg-transparent p-4 md:p-5">
        <div className="flex flex-col items-start gap-2 pb-2 sm:flex-row sm:items-center sm:gap-4 md:pb-3">
          ...
          {showSearch && (
            <input
              ...
              className="relative block h-[46px] w-full appearance-none rounded-[5px] border border-[#00374A] bg-transparent px-3 py-3 text-white shadow-none placeholder:text-xs placeholder:text-[#FFFFFF66] focus:border-[#20C5FE] focus:ring-0 focus:ring-transparent focus:outline-none sm:w-auto sm:flex-1 sm:text-sm md:px-4 md:text-[14px] md:placeholder:text-sm lg:h-[55px]"
            />
          )}
        </div>
        <div
            ...
            {loadingCurrencies ? (
              ...
              visibleCoins.map((c) => (
                <button
                  ...
                  className={`group flex min-h-[80px] w-full items-center justify-between gap-3 rounded-[5px] border border-[#00374A] bg-transparent px-4 py-4 text-[#cfd8df] transition-all duration-300 hover:shadow-[0_5px_30px_0_rgba(32,197,254,0.46)_inset] focus:border-[#20C5FE] sm:px-4 ${
                    selectedCurrency === c.code
                      ? 'border-[#20C5FE] shadow-[0_5px_30px_0_rgba(32,197,254,0.46)_inset]'
                      : ''
                  }`}
                >
                  ...
                  {c.icon ? (
                    ...
                    <div
                      ...
                      style={{ backgroundColor: c.color || '#20C5FE' }}
                    >
