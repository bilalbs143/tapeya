...
export default function DepositBankTab() {
  ...
  const PaymentMethodCard = ({ method, methodType }) => (
    <button
      ...
      className="flex h-[64px] w-full cursor-pointer items-center justify-center rounded-lg border border-[#20C5FE] bg-white transition-all duration-300 hover:border-[#20C5FE] hover:shadow-[0_5px_30px_0_rgba(32,197,254,0.46)_inset] sm:h-[75px]"
    >
    ...
    return (
      <div className="mb-3 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-[#20C5FE] md:text-sm">
            ({approvalType})
          </span>
        </div>
        ...
        {!isLast && (
          <div className="py-4">
            <div className="border-opacity-50 w-full border-b border-[#FFFFFF40]" />
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="space-y-6 overflow-hidden">
      ...
      <div className="mt-8">
        <h3 className="mb-4 text-[20px] font-bold text-white md:text-[22px]">
          {t('deposit_history')}
        </h3>
        <DepositHistory />
      </div>
