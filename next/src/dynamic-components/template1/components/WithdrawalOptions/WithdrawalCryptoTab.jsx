...
import CryptoWithdrawalDetails from '@/dynamic-components/template1/components/CryptoWithdrawalDetails/CryptoWithdrawalDetails';
...
export default function WithdrawalCryptoTab() {
  ...
  useEffect(() => {
    const baseCurrency = currency?.includes('-') ? 'usdc' : currency;
    if (
      addressVerificationData &&
      addressVerificationData.address === address &&
      addressVerificationData.currency === baseCurrency
    ) {
  ...
  const verifyButtonClassName = useMemo(() => {
    ...
    return `${baseClasses} bg-[#5343B1] text-white hover:bg-[#FC7E09]`;
  }, [addressVerificationStatus]);
          ...
          <Controller
            ...
            render={({ field }) => (
              <Input
                ...
                className="relative block w-full appearance-none rounded-[12px] border border-[#5343B1] bg-[#312577] px-3 py-3 text-white placeholder:text-[#B3A6FF] focus:z-10 focus:border-[#FC7E09] focus:ring-1 focus:ring-[#FC7E09] focus:outline-none sm:text-sm"
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <button
              ...
              className="btn-hover-fill inline-flex h-[46px] items-center justify-center rounded-[8px] px-6 text-[14px] text-white transition-opacity disabled:cursor-not-allowed disabled:opacity-50"
            >
              {renderButtonContent()}
            </button>
          </div>
