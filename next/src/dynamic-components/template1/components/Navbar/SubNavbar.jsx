...
function SubNavbar() {
  ...
  const [now, setNow] = useState(() => formatDateTimeWithSeconds(new Date()));

  useEffect(() => {
    const id = setInterval(() => {
      setNow(formatDateTimeWithSeconds(new Date()));
    }, 1000);
  ...
  // const announcementText =
  //   importantAnnouncementsData?.content || t('welcome_to_kokobet777');
  ...
  return (
    <nav className="relative z-[1] bg-[#141943]">
        ...
        <div className="hidden items-center md:grid md:[grid-template-columns:1fr_650px_1fr]">
          ...
          <div className="flex justify-center">
            <div className="flex w-full items-center gap-3 overflow-hidden rounded-full px-4 py-2 text-white/90 transition-colors hover:text-white">
              <Image
                src="https://d3emlo5tm9es2f.cloudfront.net/next/icons/announce-icon.svg"
                alt={t('announcement')}
                width={16}
                height={16}
              />
              <span className="h-4 w-px flex-shrink-0 bg-[#F25307]/60" />
              <div
          ...
          <div className="flex items-center justify-end gap-4">
            <span className="md:text[16px] text-[16px] font-medium text-[#FC9405]">
              {now}
            </span>
