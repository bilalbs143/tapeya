'use client';

import { useAuthModal } from '@/hooks/useAuthModal';
import { useTranslations } from '@/hooks/useTranslations';

export default function PendingExchangeAlert({
  pendingDepositCount = 0,
  pendingWithdrawalCount = 0,
  totalPendingCount = 0,
  hasPendingRequests = false,
}) {
  const { t } = useTranslations();
  const { openAuthModal } = useAuthModal();

  if (!hasPendingRequests) {
    return null;
  }

  const handleOpenTransactionTab = () => {
    openAuthModal({ modal: 'transaction', props: { defaultTab: 'deposit' } });
  };

  const handleOpenWithdrawalTab = () => {
    openAuthModal({
      modal: 'transaction',
      props: { defaultTab: 'withdrawal' },
    });
  };

  return (
    <div className="space-y-4">
      <div className="flex items-center space-x-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#10B981]">
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="text-white"
          >
            <path
              d="M12 1L3 5V11C3 16.55 6.84 21.74 12 23C17.16 21.74 21 16.55 21 11V5L12 1Z"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M9 12L11 14L15 10"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>
        </div>
        <div className="flex-1">
          <p className="text-sm font-medium text-[#D9D9D9]">
            {t('you_have_pending_exchange_requests')}
          </p>
          <p className="text-xs text-[#B8B8B8]">
            {totalPendingCount}{' '}
            {totalPendingCount === 1
              ? t('pending_request')
              : t('pending_requests')}
          </p>
        </div>
      </div>

      <div className="rounded-lg bg-[#1a1a2e] p-4">
        <p className="mb-2 text-sm text-[#D9D9D9]">
          {t('please_review_your_pending_exchange_requests')}
        </p>

        {pendingDepositCount > 0 && (
          <div className="flex items-center justify-between py-1">
            <span className="text-xs text-[#10B981]">
              {pendingDepositCount} {t('pending_deposits')}
            </span>
          </div>
        )}

        {pendingWithdrawalCount > 0 && (
          <div className="flex items-center justify-between py-1">
            <span className="text-xs text-[#F59E0B]">
              {pendingWithdrawalCount} {t('pending_withdrawals')}
            </span>
          </div>
        )}
      </div>

      <div className="space-y-2">
        {pendingDepositCount > 0 && (
          <button
            onClick={handleOpenTransactionTab}
            className="btn-hover-fill w-full cursor-pointer rounded-full bg-[#10B981] px-4 py-3 text-sm font-normal text-white transition-all duration-150 hover:bg-[#059669] active:scale-95 active:shadow-inner"
            data-hover="View Deposits"
          >
            {t('view_deposits')}
          </button>
        )}

        {pendingWithdrawalCount > 0 && (
          <button
            onClick={handleOpenWithdrawalTab}
            className="btn-hover-fill w-full cursor-pointer rounded-full bg-[#F59E0B] px-4 py-3 text-sm font-normal text-white transition-all duration-150 hover:bg-[#D97706] active:scale-95 active:shadow-inner"
            data-hover="View Withdrawals"
          >
            {t('view_withdrawals')}
          </button>
        )}
      </div>
    </div>
  );
}
