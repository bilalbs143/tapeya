...
export default function ExchangeTab() {
  ...
  // If user navigated here from Points/Coupons tab, set the appropriate default
  useEffect(() => {
    if (previouslySelectedTab === 'Coupons') {
      setConvertType('coupon_points_exchange');
      dispatch(clearPreviouslySelectedTab());
    }
    if (previouslySelectedTab === 'Points') {
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-4">
        <div>
          ...
          <div className="relative" ref={convertDropdownRef}>
            <div
              id="convert-select"
              className="relative flex h-[46px] w-full cursor-pointer appearance-none items-center justify-between rounded-[12px] border border-[#5343B1] bg-transparent px-3 py-3 text-[12px] text-white focus:border-[#FC7E09] focus:ring-1 focus:ring-[#FC7E09] focus:outline-none md:text-[14px]"
            ...
            {isConvertDropdownOpen && (
              <div className="scrollbar-hide absolute z-[99999] mt-1 max-h-[150px] w-full overflow-y-auto rounded-lg border border-[#5343B1] bg-[#22195A] shadow-lg">
                <div
                  className="cursor-pointer px-3 py-2 text-[12px] text-white transition-colors duration-150 hover:bg-[#FC7E09] md:text-[14px]"
                  onClick={() => {
                    setConvertType('points_exchange');
                    setIsConvertDropdownOpen(false);
                  }}
        ...
        <div>
          ...
          <div
            id="available-amount"
            className="flex h-[46px] w-full items-center rounded-[12px] border border-transparent bg-[#372A84] px-3 text-[12px] text-white md:text-[14px]"
          >
            {convertType === 'points_exchange' ? (
        ...
        <form onSubmit={handleSubmitConvert} className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <input
              ...
              className="relative block h-[46px] w-full appearance-none rounded-[12px] border border-[#5343B1] px-3 py-3 text-white placeholder:text-xs placeholder:text-[#B3A6FF] placeholder:capitalize focus:z-10 focus:border-[#FC7E09] focus:ring-1 focus:ring-[#FC7E09] focus:outline-none sm:w-auto sm:flex-1 sm:text-sm md:px-4 md:text-[14px] md:placeholder:text-sm"
              min="1"
              step="1"
            />
            <button
              ...
              className="btn-hover-fill inline-flex h-[46px] items-center justify-center rounded-[8px] px-6 text-[14px] whitespace-nowrap text-white transition-opacity disabled:cursor-not-allowed disabled:opacity-50 sm:shrink-0"
              data-hover={
