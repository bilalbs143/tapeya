...
import CryptoSelect from '@/dynamic-components/template2/components/CryptoSelect/CryptoSelect';
import CryptoWithdrawalDetails from '@/dynamic-components/template2/components/CryptoWithdrawalDetails/CryptoWithdrawalDetails';
...
export default function WithdrawalCryptoTab() {
  ...
  const verifyButtonClassName = useMemo(() => {
    ...
    return `${baseClasses} bg-[#51A2FF] text-white hover:bg-[#51A2FF]`;
  }, [addressVerificationStatus]);
      ...
      <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="space-y-2">
          ...
          >
            {renderAmountLabel()}
            {currency && (
              <span className="text-xs text-[#51A2FF]">
                {' '}
                (
                {minimumAmountLoader
                  ? t('loading')
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <button
              ...
              className="btn-style901 inline-flex h-[46px] items-center justify-center rounded-[8px] bg-[#51A2FF] px-6 text-[14px] text-white transition-opacity hover:bg-[#4a96e6] disabled:cursor-not-allowed disabled:opacity-50"
            >
              {renderButtonContent()}
            </button>
          </div>
