...
export default function DepositBankTab() {
  ...
  const PaymentMethodCard = ({ method, methodType }) => (
    <button
      ...
      className="flex h-[64px] w-full cursor-pointer items-center justify-center rounded-lg border border-[#5343B1] bg-white transition-all duration-300 hover:border-[#51A2FF] hover:shadow-[0_0_10px_0_#51A2FF_inset] sm:h-[75px]"
    >
    ...
    return (
      <div className="mb-3 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-[#51A2FF] md:text-sm">
            ({approvalType})
          </span>
        </div>
        ...
        {!isLast && (
          <div className="py-4">
            <div className="border-opacity-50 w-full border-b border-[#51a2ff30]" />
          </div>
        )}
      </div>
    );
