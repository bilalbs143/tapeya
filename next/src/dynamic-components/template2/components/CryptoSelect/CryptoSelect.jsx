...
export default function CryptoSelect({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
        ...
        <div className="mb-[10px] flex w-full flex-wrap gap-1 md:gap-2">
          {categories.map((t) => (
            <button
              ...
              className={`flex-1 rounded-[10px] border border-[#FFFFFF66] px-3 py-2 text-[12px] font-bold whitespace-nowrap transition-all duration-300 md:flex-none md:border-0 md:px-6 md:py-3 md:text-[14px] ${
                category === t.key ? 'bg-[#51A2FF] text-white' : 'text-white'
              }`}
            >
              {t.label}
            </button>
      ...
      <div className="rounded-[6px] border border-[#FFFFFF66] bg-transparent p-4 md:p-5">
            ...
            {loadingCurrencies ? (
              ...
              visibleCoins.map((c) => (
                <button
                  ...
                  className={`group flex min-h-[80px] w-full items-center justify-between gap-3 rounded-[6px] border border-[#FFFFFF66] bg-[#02040F] px-4 py-4 text-[#cfd8df] transition-all duration-300 hover:shadow-[inset_0px_0px_8px_3px_rgb(255_255_255_/_42%)] sm:px-4 ${
                    selectedCurrency === c.code
                      ? 'shadow-[inset_0px_0px_8px_3px_rgb(255_255_255_/_42%)]'
                      : ''
                  }`}
                >
