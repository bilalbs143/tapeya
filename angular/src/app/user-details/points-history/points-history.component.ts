...
export class PointsHistoryComponent implements AfterViewInit, OnInit {
  ...
  public displayedColumns: string[] = ['#', 'name', 'username', 'beforeTransaction', 'amount', 'afterTransaction', 'type', 'memo', 'createdAt'];
  ...
  private getAllCategories(): void {
    this.transactionsHistoryService.categories().subscribe({
      next: (response) => {
        this.categories =
          response.sub_types.points.categories.map((item: { [x: string]: any }) => {
