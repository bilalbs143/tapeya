...
@Component({
  selector: 'app-losing-money-history',
  templateUrl: './losing-money-history.component.html',
  styles: ``,
  standalone: false,
})
export class LosingMoneyHistoryComponent implements AfterViewInit, OnInit {
  ...
  public loadHttpData(currentPageView = this.currentPage, perPageLimit = this.pageSize): void {
    ...
    requestParams = addCreatedFilter(requestParams, this.searchForm)
      ...
      .set('filter[sub_type]', 'losing_money')
  ...
  private getAllCategories(): void {
    this.transactionsHistoryService.categories().subscribe({
      next: (response) => {
        this.categories =
          response.sub_types.losing_money.categories.map((item: { [x: string]: any }) => {
