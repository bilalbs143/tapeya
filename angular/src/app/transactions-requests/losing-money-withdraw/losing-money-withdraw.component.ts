...
@Component({
  ...
  templateUrl: './losing-money-withdraw.component.html',
  standalone: false,
})
export class LosingMoneyWithdrawComponent implements AfterViewInit, OnInit {
  ...
  public openActionDialog(type: string, _row: any): void {
    if (type === 'ACCEPT') {
      ...
      this.messageService.openDialog(LosingMoneyWithdrawActionDialogComponent, data, () => this.afterDialogClosed(), {
        widthSize: 'md',
      });
    } else {
      this.messageService.openPromptDialog(
        'LOSING_MONEY_WITHDRAWAL_REQUEST',
  ...
  public openRollingMoneyWithdrawDialog(): void {
    this.messageService.openDialog(LosingMoneyWithdrawDialogComponent, {}, () => this.afterDialogClosed(), {
      widthSize: 'md',
    });
  }
