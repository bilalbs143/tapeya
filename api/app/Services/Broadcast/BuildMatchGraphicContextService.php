...
/**
 * Computes the full graphic session context from live match data in the DB.
 *
 * Uses InningsStatsService (the single source of truth for batting/bowling/FOW
 * computation) so that live graphic data is always consistent with the
 * scorecard API.  Graphic-specific output (current-over tracking, required RR,
 * target, ball-window stats) is still built here.
 *
 * Called:
 *   - After every ball (store / update / delete) via SyncMatchGraphicContextJob
 */
final class BuildMatchGraphicContextService
{
    ...
    /**
     * Build and persist the full context, then re-broadcast the active command
     * so the overlay updates automatically. This is the standard call path for
     * the job and for any endpoint that must push a live update to the overlay.
     */
    ...
    /**
     * Merge new pending-player values, persist, then rebuild context and broadcast.
     * Only keys present in $data are written — omitted keys leave existing values intact.
     * Pass null for a key to explicitly clear that field.
     */
    public function setPendingAndBroadcast(TournamentMatch $match, array $data): MatchGraphicSession
    {
        ...
        // Keys present in $data with a non-null value → set/update the field.
        // Keys present in $data with null → explicitly clear that field.
        // Keys absent from $data entirely → leave existing value untouched.
        $merged = $existing;
    ...
    /**
     * Build and persist the full context for the match's graphic session.
    ...
    public function syncForMatch(TournamentMatch $match): MatchGraphicSession
    {
        ...
        $context = $this->build($match, $pending);
    ...
    public function build(TournamentMatch $match, array $pending = []): array
    {
        ...
        if ($innings->isNotEmpty()) {
            // Use the last in-progress innings, or the last one if all complete.
            $active = $innings->firstWhere('status', 'in_progress') ?? $innings->last();
            ...
            $context = array_merge(
                $context,
                $this->buildLive($match, $active, $first, $playerNames, $pending),
                ['innings_chart' => $this->buildInningsChart($match, $innings)],
            );
        }

        return array_merge($context, $this->tournamentLeaderboards->buildForMatch($match));
    }
     ...
     *   batters: list<array{id: int, team_id?: int, name: string, runs: int, balls: int, ones: int, twos: int, threes: int, fours: int, sixes: int, dots: int, on_strike: bool}>,
    ...
    private function graphicOverBattersBowlerStrip(
        ...
        $batters = array_map(function ($id) use ($playerNames, $batsmenById, $strikeId, $battingTeamId) {
            $id = (int) $id;
     ...
     */
    private function buildInningsChart(TournamentMatch $match, Collection $innings): array
    {
        $result = [];
