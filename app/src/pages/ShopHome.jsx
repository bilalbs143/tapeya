...
const MOST_POPULAR_PRODUCTS = [
  { id: 1, image: shopPopular1, title: 'Cricket Arabia Silver Medal for Place 34', price: '1,499', featured: true },
  { id: 2, image: shopPopular2, title: 'Cricket Arabia Silver Medal for Place 34', price: '1,499', featured: true },
  { id: 3, image: shopPopular1, title: 'Cricket Arabia Silver Medal for Place 34', price: '1,499', featured: false },
];

const SPECIAL_OFFER_PRODUCTS = [
  { id: 1, image: shopOffer1, title: 'White Athletic Shoes Blue Accents', price: '2,299', featured: true },
  { id: 2, image: shopOffer2, title: 'Sports Sunglasses Reflective', price: '1,899', featured: true },
  { id: 3, image: shopOffer1, title: 'White Athletic Shoes Blue Accents', price: '2,299', featured: false },
];
...
function ProductCard({ image, title, price, featured }) {
  return (
    <article className="flex min-w-0 w-full flex-col overflow-hidden rounded-[17px] bg-[#1a1a18]">
      <div className="relative aspect-square bg-white">
        <img src={image} alt="" className="h-full w-full object-contain p-2" />
        {featured && (
          <span className="absolute left-2 top-2 rounded-full bg-[#DA9811] px-2 py-0.5 text-[12px] font-bold uppercase text-black">
            Featured
          </span>
        )}
      </div>
  ...
  return (
    <section className="space-y-3">
      <header className="flex items-center justify-between">
        ...
        <Link
          to={viewMorePath}
          className="text-[12px] font-bold uppercase text-[#DA9811] transition-opacity active:opacity-80"
        >
          View more
        </Link>
      </header>
        ...
        <div className="flex gap-2 overflow-x-auto py-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {SHOP_BRANDS.map(({ id, label }) => (
            <Link
              key={id}
              to={`/shop/${id}`}
              className="flex shrink-0 items-center gap-2 rounded-[6px] bg-[#141412] px-[13px] py-[10px] text-[13px] font-semibold uppercase tracking-wide text-white transition-colors hover:bg-[#252520]"
