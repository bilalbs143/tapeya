          ...
          {team.batting.map((player, i) => (
            <tr key={i}>
              <td className={`border-r border-b border-l ${BORDER} px-4 py-3`}>
                ...
                {player.dismissal && <p className="mt-0.5 text-[12px] text-[#A2A6AB]">{player.dismissal}</p>}
              </td>
      ...
      <div className="mb-4 flex justify-center">
        {teams.map((team, i) => (
          <button
            ...
            className={`mr-6 text-[14px] font-normal transition-colors ${i === teamIndex ? 'text-[#DA9811]' : 'text-[#A2A6AB]'}`}
          >
            <span className={i === teamIndex ? 'inline-block border-b-4 border-[#DA9811] pb-2.5' : 'inline-block pb-3'}>
              {team.name}
            </span>
          </button>
        ))}
