...
export function SquadSingle({ tournamentId, teamId }) {
  ...
  if (isLoading) {
    return (
      <div className="mt-4 pb-6">
        <p className="py-4 text-center text-[13px] text-[#A2A6AB]">Loading squad…</p>
      </div>
    );
  }
  ...
  if (!squad.length) {
    return (
      <div className="mt-4 pb-6">
        <p className="py-8 text-center text-[13px] text-[#A2A6AB]">No players in this squad yet.</p>
      </div>
    );
  }
          ...
          {squad.map((player, index) => (
            <div key={player.id ?? index} className={`flex border-b ${BORDER_ALT} last:border-b-0`}>
              ...
              <div className="min-w-0 flex-1 px-4 py-3">
                ...
                <div className="mt-0.5 text-[12px] text-[#A2A6AB]">{playerDisplayRole(player)}</div>
              </div>
            </div>
          ))}
        </div>
