    ...
    <>
      ...
      <div className="relative z-10 flex w-full flex-col items-center px-6 pt-6 sm:pt-8 md:pt-10 lg:pt-16">
        ...
        <div className="mt-10 w-full max-w-[358px] space-y-4 lg:mt-14 lg:max-w-[880px] lg:p-0 lg:px-20">
          ...
          <p className="pt-2 text-center text-[14px] text-muted lg:pt-0">
  ...
  return (
    <>
      ...
      <div className="scrollbar-hide mb-2 flex max-h-[280px] flex-col gap-3 overflow-y-auto p-[10px]">
        {profiles.map((profile) => {
          ...
          return (
            <div
              key={profile.phone}
              className="relative flex w-full rounded-[18px] border border-surface-border bg-surface px-4 py-3.5"
            >
              <button
                type="button"
                onClick={(e) => onRemove(e, profile.phone)}
                aria-label={`Remove ${profile.name ?? profile.phone} from saved accounts`}
                className="absolute top-[5px] right-[5px] flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand text-[20px] leading-none font-bold text-ink shadow-sm transition-opacity hover:opacity-90 focus:ring-2 focus:ring-brand focus:ring-offset-2 focus:ring-offset-[#141412] focus:outline-none active:opacity-80"
              >
                ×
              </button>
              ...
              <button
                ...
                disabled={busy}
                className="flex min-w-0 flex-1 items-center gap-4 pr-8 text-left transition-opacity focus:ring-2 focus:ring-brand focus:ring-offset-2 focus:ring-offset-black focus:outline-none active:opacity-90 disabled:opacity-60"
              >
                <Avatar className="h-12 w-12 shrink-0 overflow-hidden rounded-full border border-surface-border">
                  ...
                  <AvatarFallback className="bg-brand text-sm font-bold text-ink">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[12px] font-medium text-muted">{formatPhoneMasked(profile.phone)}</p>
                </div>
              </button>
            </div>
          );
      ...
      <button
        type="button"
        onClick={onUseOther}
        className="w-full py-3 text-center text-[14px] font-medium text-brand underline underline-offset-2 transition-colors hover:text-[#E8A820]"
      >
