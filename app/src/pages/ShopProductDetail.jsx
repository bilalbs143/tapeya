...
export default function ShopProductDetail() {
  ...
  const categoryLabel = (state?.category ?? product.category ?? 'Product').toUpperCase();
    ...
    <Container>
      <div className="flex flex-col gap-4">
        ...
        <header className="flex -mx-4 -mt-6 px-4 pt-6 pb-4 items-center gap-3 bg-black">
          ...
          <h1 className="min-w-0 flex-1 text-[16px] font-bold uppercase text-center tracking-wide text-white truncate pr-4">
            SHOP - {categoryLabel}
          </h1>
        </header>
        ...
        <div className="space-y-3">
          <div className="relative overflow-hidden rounded-[17px] bg-white">
            <img src={images[selectedImage]} alt="" className="aspect-square w-full object-contain p-4" />
            {product.featured && (
              <span className="absolute left-3 top-3 rounded-full bg-[#DA9811] px-2.5 py-1 text-[12px] font-bold uppercase text-black">
                Featured
              </span>
            )}
          </div>
            ...
            {images.map((img, i) => (
              <button
                ...
                className={`h-12 w-12 shrink-0 overflow-hidden rounded-full border-2 bg-white ${
                  selectedImage === i ? 'border-[#DA9811]' : 'border-transparent'
                }`}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-4">
            {product.originalPrice && (
              <span className="text-[16px] text-[#A2A6AB82] font-bold line-through">PKR {product.originalPrice}</span>
            )}
            <span className="text-[16px] font-bold text-[#DA9811]">PKR {product.price}</span>
          </div>
          <p className="text-[12px] font-bold text-[#A2A6AB]">
            Availability: <span className="text-[#FF3B30] ml-2 text-[12px]">Only {product.stock ?? 1} left in stock</span>
          </p>
        </div>
        ...
        <section className="space-y-2 pt-2">
          <h3 className="text-[12px] font-bold uppercase tracking-wide text-[#A2A6AB]">Features</h3>
          <ul className="space-y-1 text-[12px] text-[#A2A6AB]">
            {(product.features ?? DEFAULT_PRODUCT.features).map((line, i) => (
              <li key={i} className="flex gap-2">
                <span className="text-[#A2A6AB] shrink-0">*</span>
