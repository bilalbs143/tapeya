...
const EMPTY_INNINGS = { partnerships: { completed: [], current: null } };

export function PartnershipTab({ innings1 = EMPTY_INNINGS, innings2 = EMPTY_INNINGS }) {
    ...
    <div className="mt-4 flex flex-col pb-8">
      ...
      <div className="mt-4 flex flex-col gap-0">
        {allPartnerships.length === 0 ? (
          <p className="py-6 text-center text-[13px] text-[#A2A6AB]">No partnerships yet.</p>
        ) : (
          allPartnerships.map((p, index) => (
            ...
            >
              ...
              <div className="flex min-w-0 flex-1 items-center gap-2">
                <span className="truncate text-[12px] font-semibold text-[#A2A6AB]">{p.batter1.name}</span>
              ...
              <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
                ...
                <div className={VERTICAL_SEPARATOR} aria-hidden />
                <span className="truncate text-[12px] font-semibold text-[#A2A6AB]">{p.batter2.name}</span>
              </div>
            </div>
          ))
        )}
