...
const STATUS_PILL_STYLES = {
  pending: 'border border-[#DA9811] text-[#DA9811] font-bold uppercase tracking-wide',
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        {relativeTime && <span className="shrink-0 text-[12px] font-medium text-[#A2A6AB]">{relativeTime}</span>}
      </div>
      <div className="mt-2 flex flex-wrap items-start justify-between gap-2">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <p className="text-base text-[16px] leading-tight font-bold text-[#DA9811]">{formatPrice(order.total)}</p>
      ...
      <div className="mt-1.5 flex min-w-0 flex-wrap items-center gap-1">
        ...
        {extraCount > 0 && <span className="shrink-0 font-normal text-[#DA9811]">+{extraCount}</span>}
      </div>
    </button>
  );
});
      ...
      <Container>
        ...
        {isError && !isLoading && (
          <div className="flex min-h-[30vh] flex-col items-center justify-center gap-4 py-12">
            <p className="text-[14px] text-[#A2A6AB]">Could not load orders. Please try again.</p>
            <button
              type="button"
              onClick={() => refetch()}
        ...
        {!isLoading && !isError && (
          <div className="flex flex-col gap-8">
            <section>
              <h2 className="mb-4 text-[13px] font-bold tracking-wide text-[#A2A6AB] uppercase">CURRENT ORDERS</h2>
              <div className="flex flex-col gap-3">
                {currentOrders.length === 0 ? (
                  <p className="text-[13px] text-[#A2A6AB]">No current orders.</p>
                ) : (
            ...
            <section>
              <h2 className="mb-4 text-[13px] font-bold tracking-wide text-[#A2A6AB] uppercase">PREVIOUS ORDERS</h2>
              <div className="flex flex-col gap-3">
                {previousOrders.length === 0 ? (
                  <p className="text-[13px] text-[#A2A6AB]">No previous orders.</p>
                ) : (
