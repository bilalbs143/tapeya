  ...
  return (
    <section className="mb-10 space-y-3">
      <header className="flex items-center justify-between">
        ...
        <Link to={viewMorePath} className="text-[12px] font-bold text-brand uppercase transition-opacity active:opacity-80">
          View More
        </Link>
      </header>
      <Swiper
              ...
              <div className="flex gap-2 overflow-x-auto py-1 [scrollbar-width:none] lg:gap-3 [&::-webkit-scrollbar]:hidden">
                {brands.map(({ id, name, slug, logo }) => (
                  <Link
                    key={id}
                    to={`/shop/${slug}`}
                    className="flex shrink-0 items-center gap-2 rounded-[6px] bg-surface px-[13px] py-[10px] text-[13px] font-semibold tracking-wide text-white uppercase transition-colors hover:bg-[#252520] lg:min-w-[96px] lg:justify-center lg:px-4"
