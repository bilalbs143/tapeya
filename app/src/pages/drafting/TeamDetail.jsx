  ...
  if (!hasValidTeamId) {
    return (
      <div className="bg-black">
        ...
        <Container>
          <p className="py-6 text-center text-[13px] text-muted">Invalid team.</p>
        </Container>
      </div>
    );
  }
  ...
  if (isLoadingTeams && !team) {
    return (
      <div className="bg-black">
        <AppSubpageHeader title="Drafting" />
        <Container>
          <p className="py-6 text-center text-[13px] text-muted">Loading team…</p>
        </Container>
      </div>
    );
  }
  ...
  if (!team) {
    return (
      <div className="bg-black">
        <AppSubpageHeader title="Drafting" />
        <Container>
          <p className="py-6 text-center text-[13px] text-muted">Team not found.</p>
          <button
            type="button"
            onClick={() => navigate('/drafting/teams')}
            className="mx-auto mt-2 block text-[13px] font-semibold text-brand"
          >
            Back to Teams
          </button>
        </Container>
    ...
    <div className="bg-black">
      ...
      <Container>
        <div className="lg:grid lg:grid-cols-2 lg:gap-6">
          <div>
            <p className="mb-3 text-[13px] font-medium tracking-wide text-muted uppercase">{team.name?.toUpperCase()}</p>
            ...
            <div className="mb-4 flex flex-col gap-1">
              ...
              <div className="relative">
                <input
                  ...
                  className="h-12 w-full rounded-[6px] bg-surface py-3 pr-12 pl-4 text-white placeholder:text-base placeholder:text-muted/47 focus:ring-2 focus:ring-brand/50 focus:outline-none disabled:opacity-60"
                ...
                {findPlayer && (
                  <button
                    ...
                    className="absolute inset-y-0 right-10 flex w-8 items-center justify-center text-muted hover:text-white"
              ...
              <tbody>
                {!isLoadingSquad && squad.length === 0 && (
                  <tr>
                    <td
                      colSpan={3}
                      className={`border-r border-b border-l py-6 text-center text-[13px] text-muted ${BORDER}`}
                    >
                      {squadEmptyMessage}
                    </td>
                  </tr>
        ...
        <button
          type="button"
          onClick={() => navigate('/drafting/teams')}
          className="m-auto flex h-12 max-w-fit items-center justify-center rounded-[6px] border border-brand px-4 text-center text-[16px] font-bold tracking-wide text-brand uppercase transition-opacity active:opacity-90 lg:m-0"
        >
          Back to Teams
        </button>
      </Container>
