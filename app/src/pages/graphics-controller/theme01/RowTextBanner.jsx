            ...
            {[0, 1].map((copyIndex) => (
              <div key={copyIndex} className="flex h-full w-full shrink-0 gap-1">
                {mobileItems.map((_, itemIndex) => (
                  ...
                  >
                    <span
                      className={`pointer-events-none absolute inset-0 flex items-center justify-center font-moul leading-none text-[#FFFFFF0A] uppercase ${mobileShadowClass}`}
                    >
                      {text}
                    </span>
                    <span className={`relative font-moul leading-none text-white uppercase ${mobileTextClass}`}>
                      {text}
                    </span>
                  </div>
                ))}
            ...
            {[0, 1].map((copyIndex) => (
              <div key={copyIndex} className="flex h-full w-full shrink-0 gap-3">
                {desktopItems.map((_, itemIndex) => (
                  ...
                  >
                    <span className="pointer-events-none absolute inset-0 flex items-center justify-center font-moul text-[54px] leading-none text-[#FFFFFF0A] uppercase">
                      {text}
                    </span>
                    <span className="relative font-moul text-[36px] leading-none text-white uppercase">
                      {text}
                    </span>
                  </div>
                ))}
