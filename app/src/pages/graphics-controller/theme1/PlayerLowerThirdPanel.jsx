 ...
 */
export default function PlayerLowerThirdPanel({ playerName = '', headerRight = null, stats = [] }) {
  return (
    <div className={`relative min-h-screen overflow-hidden ${PLAYER_PAGE_BG_CLASS}`}>
      <section className="absolute right-0 bottom-0 left-0 text-white">
        <div className="relative w-full overflow-hidden px-3 py-2 sm:px-10 sm:py-4" style={controllerPanelSurfaceStyle()}>
          <div className="flex items-start justify-between gap-2 sm:gap-6">
            <p className={`min-w-0 flex-1 ${playerNameAccentClass}`}>{playerName}</p>
            {headerRight != null && headerRight !== false ? <div className="shrink-0 text-right">{headerRight}</div> : null}
          </div>
          <div className="mt-2 h-px w-full sm:mt-3" style={playerGraphicHorizontalRuleStyle} />
