...
export default function EditSquad() {
  ...
  const filteredPlayers = useMemo(() => {
    if (!findPlayer.trim()) return players;
      ...
      <Container className="!px-4 !py-0">
        <header className="-mx-4 -mt-6 flex items-center gap-3 bg-black px-4 pt-6 pb-6">
          ...
          <h1 className="min-w-0 flex-1 pr-[27px] text-center text-[16px] font-bold tracking-wide text-white uppercase">
            Organizer - Edit Squad
          </h1>
        </header>

        <p className="mb-3 text-[13px] font-medium uppercase tracking-wide text-white">
        ...
        <div className="mb-4">
          <label htmlFor="find-player" className="mb-2 block text-[14px] font-medium text-white">
            Find Player
          </label>
        ...
        <div className="mb-5">
          <label htmlFor="nickname" className="mb-2 block text-[14px] font-medium text-white">
            Nickname
          </label>
          <input
            id="nickname"
          ...
          <table className="w-full border-collapse text-[12px] text-white">
            <thead>
              <tr className={HEADER_BG}>
                ...
                <th
                  className={`${HEADER_BG} border-b border-r py-2.5 text-center font-bold text-white ${BORDER}`}
                >
                  Playing Role
                </th>
                <th
            ...
            <tbody>
              {filteredPlayers.map((player, index) => (
                <tr key={player.id}>
                  ...
                  <td
                    className={`border-b border-r py-3 text-center text-white ${BORDER}`}
                  >
                    {player.role}
                  </td>
                  <td
                    className={`border-b border-r py-3 pr-4 text-right ${BORDER}`}
                  >
                    <button
                      type="button"
        ...
        <button
          ...
          className="flex h-12 max-w-fit m-auto items-center justify-center rounded-[6px] border-2 border-[#DA9811] px-4 text-[16px] font-bold uppercase tracking-wide text-[#DA9811] transition-opacity active:opacity-90"
        >
          Submit Squad
        </button>
      </Container>
