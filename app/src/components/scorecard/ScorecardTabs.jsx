...
export function ScorecardTabs({ matches, tournaments = [], fixedVisible = false, fixedTop = 64 }) {
  return (
    <Tabs defaultValue="all" className="w-full">
      {fixedVisible && (
        <div className="fixed right-0 left-0 z-40 bg-black pt-1 pb-2 lg:left-[280px]" style={{ top: fixedTop }}>
      ...
      <TabsContent value="all" className="mt-4 focus:outline-none">
        {matches.length === 0 ? (
          <p className="py-8 text-center text-[13px] text-[#A2A6AB]">No matches in this category</p>
        ) : (
