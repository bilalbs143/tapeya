...
export function ScoringTableAddButton({ label, onClick, className = '' }) {
  ...
  return (
    <Button
      ...
      variant="dark"
      size="md"
      className={`flex h-[72px] w-[140px] flex-col items-center justify-center gap-1.5 px-0 py-0 ${className}`.trim()}
      aria-label={addLabel}
      onClick={onClick}
    >
      ...
      <span className="text-muted text-[13px] font-bold tracking-wide">{addLabel}</span>
    </Button>
  );
}
