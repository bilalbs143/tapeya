...
export function ExploreCategories() {
  return (
    <section className="space-y-4">
      <h2 className="text-[13px] font-bold tracking-wide text-muted uppercase md:text-[16px]">
        <span>Explore</span>
      </h2>
      <div className="grid grid-cols-4 gap-3 md:grid-cols-5 md:gap-4">
        {EXPLORE_CATEGORIES.map(({ path, label }) => (
          <Link
            key={path}
            to={path}
            className="flex min-h-[90px] w-full flex-col items-center justify-center gap-2 rounded-[17px] bg-surface transition-colors duration-200 ease-out hover:bg-surface-raised active:opacity-90 md:h-auto md:min-h-[72px] md:flex-row md:items-center md:gap-3 md:px-4 md:hover:bg-[#2C2C2C]"
          >
            <img src={PATH_TO_ICON[path]} alt="" className="h-5 w-5 shrink-0 object-contain md:h-8 md:w-8" />
