...
function normalizeSummarySource(entry) {
  ...
  const teamCode = entry.teamCode ?? entry.batting_team ?? entry.battingTeam;
  if (!teamCode) return null;

  const runs = toNum(entry.runs) ?? toNum(entry.total) ?? 0;
  const wkts = toNum(entry.wickets) ?? toNum(entry.wkts) ?? 0;

  return {
    teamCode: String(teamCode),
    total: runs,
    wkts,
    scoreSep: entry.scoreSep ?? '-',
    oversText: String(entry.oversText ?? entry.overs ?? entry.overs_display ?? ''),
    oversLabel: entry.oversLabel ?? 'OVER',
  };
}
...
export function buildMatchSummaryInnings(snapshot, payloadStats) {
  const fromPayload = Array.isArray(payloadStats) ? payloadStats.map(normalizeSummarySource).filter(Boolean) : [];
  const fromContext = (snapshot.live.inningsSummaries ?? []).map(normalizeSummarySource).filter(Boolean);
