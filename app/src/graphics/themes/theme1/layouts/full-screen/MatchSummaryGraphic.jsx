...
import { batterScore, colors, fsMatchSummary, fsSummaryPanel } from '../../config';
import {
  accentMix,
...
const MATCH_SUMMARY_GOLD = colors.gold;
...
const rowNameMdClass = cn(
  'min-w-0 flex-1 overflow-hidden text-ellipsis whitespace-nowrap font-bold text-white uppercase',
  DISPLAY_FONT,
);
      ...
      <div className="flex" style={{ gap: STATS_COLUMNS_GAP }}>
        <div className="min-w-0 flex-1">
          {Array.from({ length: rows }).map((_, index) => {
            ...
            return (
              <div key={`bat-${index}`} className="flex h-[50px] items-center border-b border-white/8">
                {batter ? (
                  <BattingMiniRow
                    name={batter.name ?? ''}
                    runs={batter.runs}
        ...
        <div className="min-w-0 flex-1">
          {Array.from({ length: rows }).map((_, index) => {
            ...
            return (
              <div key={`bowl-${index}`} className="flex h-[50px] items-center border-b border-white/8">
                {bowler ? (
...
function NeedTargetBand({ label }) {
  return (
    <div
      ...
      style={{
        background: `linear-gradient(100deg, ${MATCH_SUMMARY_GOLD}, ${colors.goldDark})`,
