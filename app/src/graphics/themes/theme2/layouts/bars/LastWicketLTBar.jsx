...
import { fmt, formatBatterBalls, InsetLTAnimatedNumber, UI_FONT } from '../../primitives';
...
const nameClass = cn(
  ...
  'font-bold leading-none text-[var(--text)] uppercase',
  UI_FONT,
);

const dismissalClass = cn(
  'min-w-0 truncate',
  'text-[15px] font-semibold leading-none tracking-[0.06em] text-[var(--text)] uppercase',
  UI_FONT,
);
      ...
      header={({ measuring }) => (
        <>
          ...
          <span className={scoreClusterClass} style={{ gap: ltPlayerStatBar.scoreGap }}>
            ...
            >
              {formatBatterBalls(player.balls ?? 0)}
            </span>
          </span>
        </>
      )}
