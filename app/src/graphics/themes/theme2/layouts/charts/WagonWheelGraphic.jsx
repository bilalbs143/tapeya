...
const ZONE_BAR_COLORS = [
  colors.fourTitleColor,
  colors.gold,
  '#5fd0a8',
  colors.sixStrapAccent,
  colors.accentA,
  colors.accentB,
  '#f0d44a',
  '#ff8a65',
...
function WagonWheelField({ shots, size = WHEEL_SIZE, animate = true }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible" aria-hidden="true">
      ...
      <circle cx={cx} cy={cy} r="6" fill="#fff" stroke={colors.accentA} strokeWidth="2" />
    </svg>
  );
}
...
function WagonBigStat({ label, value, accent, dense = false }) {
  return (
    <div className="rounded px-5 py-4" style={{ background: colors.panelPlayer }}>
  ...
  return (
    <div className={cn('flex flex-col', denseZones ? 'gap-4' : 'gap-[26px]')}>
      ...
      <div className="grid grid-cols-3 gap-3.5">
        <WagonBigStat label="RUNS" value={stats.total} accent={colors.text} dense={denseZones} />
        <WagonBigStat label="FOURS" value={stats.fours} accent={SHOT_COLORS.four} dense={denseZones} />
        <WagonBigStat label="SIXES" value={stats.sixes} accent={SHOT_COLORS.six} dense={denseZones} />
      </div>
...
export function WagonWheelGraphic({ data }) {
  const { title, sub, logoUrl, teams, player, shots, zoneBreakdown = [] } = data;
    ...
    <FSStage>
      ...
      >
        <GlowPanel
          ...
          style={{ gridTemplateColumns: `${WHEEL_COLUMN_W}px 1fr`, background: 'rgba(46, 10, 26, 0.8)' }}
        >
