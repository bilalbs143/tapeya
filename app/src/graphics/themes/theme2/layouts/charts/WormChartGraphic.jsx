...
/** Gold milestone markers — theme3 worm reference. */
const WORM_MARKER_COLOR = colors.accentB;
...
function MiniStat({ label, value, accent }) {
  return (
    ...
    >
      <span className={cn('text-[20px] font-semibold tracking-[0.1em] uppercase', TEXT_PRIMARY, UI_FONT)}>{label}</span>
      <span className={cn('text-[34px] font-extrabold text-[var(--text)]', DISPLAY_FONT)}>{value}</span>
    </div>
  );
}
      ...
      >
        <span
          className={cn(
            ...
            'text-ellipsis whitespace-nowrap text-[var(--text)] uppercase',
            DISPLAY_FONT,
          )}
        >
          {name}
        </span>
        <span className={cn('px-[18px] text-[36px] font-extrabold whitespace-nowrap text-[var(--text)]', DISPLAY_FONT)}>
          {meta.total}
        </span>
        <span className="flex h-full flex-col justify-center px-[22px]" style={{ background: accentMix(accent, 16) }}>
          <span className={cn('text-[16px] font-semibold tracking-[0.1em] text-[var(--text)] uppercase', UI_FONT)}>OVERS</span>
          <span className={cn('text-[30px] font-extrabold text-[var(--text)]', DISPLAY_FONT)}>{meta.overs}</span>
        </span>
      </div>
...
export function WormChartGraphic({ data }) {
  const { title, sub, logoUrl, teams, chart, meta } = data;
  ...
  return (
    <FSStage>
      <ChartHeader title={title} sub={sub} logoUrl={logoUrl} />
      ...
      <div className="absolute right-[400px] bottom-11 left-[70px] flex gap-7">
