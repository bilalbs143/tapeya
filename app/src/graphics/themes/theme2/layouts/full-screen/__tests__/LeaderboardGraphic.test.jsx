...
describe('LeaderboardGraphic', () => {
  ...
  it('renders the featured portrait on the Top Bowler plate', () => {
    const { container } = render(<LeaderboardGraphic {...leaderboardFixture} />);
    ...
    expect(screen.getByRole('img', { name: 'Star Batter' }).getAttribute('src')).toBe('https://cdn.example/star-batter.jpg');
  });

  it('shows plate lining on the featured portrait when avatar is missing', () => {
    const { container } = render(
      <LeaderboardGraphic {...leaderboardFixture} data={{ ...leaderboardFixture.data, avatarUrl: null }} />,
    );
