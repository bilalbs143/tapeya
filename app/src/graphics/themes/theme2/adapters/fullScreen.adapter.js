...
export function toBattingSummaryBundle(props, tokens) {
  ...
  const theme2Team = toTeamRecord(battingTeam, code, tokens, side);
  ...
  return {
    ...
    data: {
      teamCode: code,
      title: battingTeam.name ?? theme2Team.displayName,
      sub: tournamentSub(props),
...
export function toBowlingSummaryBundle(props, tokens) {
  ...
  const theme2Team = toTeamRecord(bowlingTeam, code, tokens, side);
  const teams = { [code]: theme2Team };
  ...
  return {
    ...
    data: {
      teamCode: code,
      title: bowlingTeam.name ?? theme2Team.displayName,
      sub: tournamentSub(props),
      accent: theme2Team.color,
...
export function toInningFiguresBundle(props, tokens) {
  if (props.title && props.score != null && typeof props.innings === 'string' && !props.matchHeader) {
    ...
    return {
      ...
      data: {
        ...
        title: props.title,
        sub: tournamentSub(props),
        logoUrl: props.logoUrl ?? team.logoUrl ?? null,
        score: props.score,
  ...
  const theme2Team = teams?.[code] ?? toTeamRecord(battingTeam, code, tokens, props.battingTeamSide ?? null);

  return {
    teams: teams ?? { [code]: theme2Team },
    data: {
      teamCode: code,
      accent: theme2Team.color,
      title: props.matchHeader ?? '',
      sub: tournamentSub(props),
      logoUrl: props.teamLogoUrl ?? battingTeam.logoUrl ?? null,
...
function mapPartnershipBatterRow(batter, options = {}, tokens) {
  ...
  return {
    ...
    onStrike: Boolean(batter.onStrike),
...
export function toPartnershipBundle(props, tokens) {
  ...
  return {
    ...
    data: {
      ...
      teamCode: code,
      accent: theme2Team.color,
      logoUrl: props.teamLogoUrl ?? battingTeam.logoUrl ?? theme2Team.logoUrl ?? null,
      defaultAvatarUrl: props.defaultAvatarUrl ?? assets.playerPlaceholder,
      partnership: {
...
function mapPartnershipHistory(partnerships, accent, tokens) {
  return partnerships
    .map((entry) => {
      ...
      const mapped = batters.map((b, index) => ({
      ...
      const ballsFromBatters = mapped.reduce((sum, b) => sum + (Number(b.balls) || 0), 0);

      return {
        balls: entry.balls ?? entry.partnershipBalls ?? ballsFromBatters,
        batters: mapped,
      };
    })
...
export function toPartnershipListBundle(props, tokens) {
  ...
  const battingTeam = props.battingTeam ?? {};
  const code = 'batting';
  const theme2Team = toTeamRecord(battingTeam, code, tokens, props.battingTeamSide ?? null);
  const teams = { [code]: theme2Team };
  const partnerships = mapPartnershipHistory(rawPartnerships, theme2Team.color, tokens);
  ...
  return {
    ...
    data: {
      ...
      sub: tournamentSub(props),
      accent: theme2Team.color,
      crestLogoUrl: props.teamLogoUrl ?? battingTeam.logoUrl ?? theme2Team.logoUrl ?? null,
...
export function toMatchSummaryBundle(props, tokens) {
  ...
  if (props.crests && hasMatchSummaryPreviewInnings(props)) {
    ...
    return {
      ...
      data: {
        sub: tournamentSub(props),
        title: props.title ?? 'MATCH SUMMARY',
        logoUrl: props.tournamentLogoUrl ?? assets.brandLogoWhite,
