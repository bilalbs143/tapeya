...
function chartLogoUrl(props) {
  return props.tournamentLogoUrl ?? props.logoUrl ?? assets.brandLogoWhite;
}
 ...
 * @param {Record<string, object>|null} theme2Teams
 */
function chartTeamPairFromInnings(inningsTeams, tokens, homeTeam, awayTeam, theme2Teams) {
  const fixturePair = chartTeamPair(homeTeam, awayTeam, theme2Teams);
...
function resolveChartTeamPair(props, tokens) {
  const theme2Teams = toTeams(props, tokens);
  return chartTeamPairFromInnings(props.teams, tokens, props.homeTeam, props.awayTeam, theme2Teams);
}
