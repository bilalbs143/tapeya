...
export default function TOUR_HUNDREDS({ isOverlay, tokens, ...props }) {
  ...
  return (
    <BroadcastShell stage="bar">
      <div className="bc-controller-stacked flex w-full max-w-full flex-col items-start">
        <TourHitsMiniBar mini={frame.mini} defaultTitle="HUNDREDS" edgeToEdge={isOverlay} />
        <LowerThirdBar tokens={tokens} edgeToEdge={isOverlay} frame={frame} teams={teams} match={match} barVariant="hundreds" />
      </div>
    </BroadcastShell>
  );
}
