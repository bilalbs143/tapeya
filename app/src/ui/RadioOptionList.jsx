...
export function RadioOptionList({ options, value, onChange, ariaLabel, className = '' }) {
  return (
    <ul className={`flex flex-col gap-2 ${className}`} role="radiogroup" aria-label={ariaLabel}>
      {options.length === 0 && <li className="text-[13px] text-[#A2A6AB]">Loading options…</li>}
      {options.map((opt) => {
        ...
        return (
          <li key={opt.value}>
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-[10px] border-2 px-4 py-3 text-left text-[13px] text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#DA9811] ${
                selected ? 'border-[#DA9811] bg-[#1C1C1A]' : 'border-[#141412] bg-[#141412]'
              }`}
            >
              <span
                className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2 ${
                  selected ? 'border-[#DA9811]' : 'border-[#A2A6AB]'
                }`}
                aria-hidden
              >
                {selected ? <span className="h-2 w-2 rounded-full bg-[#DA9811]" /> : null}
              </span>
              {opt.label}
            </button>
          </li>
