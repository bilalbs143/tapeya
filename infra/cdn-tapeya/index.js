//
// Private Backblaze B2 → public CDN via signed S3 GETs.
// Based on https://github.com/backblaze-b2-samples/cloudflare-b2
//
...
/** Default when B2 object/bucket info has no Cache-Control (Workers Cache uses this). */
...
const CORS_HEADERS = {
    ...
    'Access-Control-Expose-Headers': 'Accept-Ranges, Content-Length, Content-Range, Content-Type, ETag, Cache-Control, Age',
...
function withResponseHeaders(response) {
    ...
    if (response.ok && !headers.has('cache-control') && !headers.has('cdn-cache-control')) {
        headers.set('Cache-Control', DEFAULT_CACHE_CONTROL);
    }

    // Avoid caching errors at the edge.
...
function createHeadResponse(response) {
    const prepared = withResponseHeaders(response);
...
export default {
    async fetch(request, env) {
        ...
        if (rcloneDownload) {
            ...
            // Normalize path (strip trailing slash) for a stable S3 key / cache key.
