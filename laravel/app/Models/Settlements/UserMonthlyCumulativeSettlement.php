...
use App\Enums\Time\MonthEnum;
use App\Models\BaseModel;
use App\Models\User;
use Spatie\QueryBuilder\AllowedFilter;

class UserMonthlyCumulativeSettlement extends BaseModel
{
    protected $fillable = [
        'user_id',
        'month',
    ...
    public function scopeFilterByAgent($q)
    {
        $q->when(request('agent_id'), function ($query) {
            $agent = User::agent()->with('grand_children:id,parent_id')->find(request('agent_id'));
            if ($agent) {
                $query->whereIn('user_id', $agent->allMemberIds());
            } else {
                $query->whereRaw('1 = 0');
            }
        });
