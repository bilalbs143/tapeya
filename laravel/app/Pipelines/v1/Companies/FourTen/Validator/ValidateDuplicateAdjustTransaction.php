...
class ValidateDuplicateAdjustTransaction
{
    /**
    ...
    public function handle(Collection $collection, Closure $next)
    {
        ...
        // Adjust can be either credit or debit, so check both
        $existingTransaction = Transaction::byTxnId($request->getReference() ?? $request->getTransactionId())->first();
