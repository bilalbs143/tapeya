...
namespace App\Http\Resources\v1\ExchangeRequest;
...
class ExchangeRequestResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'approved_at' => $this->when(! auth()->user()->isMember(), $this->approved_at),
            'approver' => $this->when(! auth()->user()->isMember(), new OperatorResource($this->approver)),
            'rejector' => $this->when(! auth()->user()->isMember(), new OperatorResource($this->rejector)),
            'rejected_at' => $this->when(! auth()->user()->isMember(), $this->rejected_at),
            'via' => $this->via?->label(),
            'bank_account' => new BankAccountResource($this->bank_account),
            'transaction_number' => $this->transaction_number,
            'receipt_path' => $this->receipt_path,
            'via_enum' => $this->via?->name,
