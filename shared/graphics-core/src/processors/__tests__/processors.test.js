...
describe('PLAYING_11 processor', () => {
  ...
  it('backfills captain/wicket-keeper flags onto an explicit payload XI from the live squad by name', () => {
    const snapshot = createTestSnapshot({
      active_command: {
        command_key: 'PLAYING_11',
        command_type: 'FULL_SCREEN',
        ...
        payload: {
          home_team: {
            players: [
              { name: 'Jahangir Khan', playing_role: 'Batsman' },
              { name: 'Salman Mirza', playing_role: 'Bowler' },
              { name: 'Zaman Khan', playing_role: 'Batsman' },
            ],
          },
          away_team: { players: [{ name: 'Sarfaraz Ahmed', playing_role: 'Batsman' }] },
        },
      },
      context: {
        squad_home: [
          { display_name: 'Jahangir Khan', is_captain: true, is_wicket_keeper: false },
          { display_name: 'Salman Mirza', is_captain: false, is_wicket_keeper: true },
          { display_name: 'Zaman Khan', is_captain: false, is_wicket_keeper: false },
        ],
        squad_away: [{ display_name: 'Sarfaraz Ahmed', is_captain: false, is_wicket_keeper: true }],
      },
    });

    const props = processPlaying11(snapshot);

    expect(props.homeTeam.players[0]).toMatchObject({ name: 'Jahangir Khan', is_captain: true, is_wicket_keeper: false });
    expect(props.homeTeam.players[1]).toMatchObject({ name: 'Salman Mirza', is_captain: false, is_wicket_keeper: true });
    expect(props.homeTeam.players[2]).toMatchObject({ name: 'Zaman Khan', is_captain: false, is_wicket_keeper: false });
    expect(props.awayTeam.players[0]).toMatchObject({ name: 'Sarfaraz Ahmed', is_captain: false, is_wicket_keeper: true });
  });

  it('leaves a payload player untouched when no squad record matches its name', () => {
    const snapshot = createTestSnapshot({
      active_command: {
        command_key: 'PLAYING_11',
        command_type: 'FULL_SCREEN',
        display_mode: 'FS',
        payload: { home_team: { players: [{ name: 'Unknown Player' }] }, away_team: { players: [{ name: 'Player B' }] } },
      },
      context: { squad_home: [{ display_name: 'Someone Else', is_captain: true }] },
    });

    const props = processPlaying11(snapshot);

    expect(props.homeTeam.players).toEqual([{ name: 'Unknown Player' }]);
  });
});
