import { resolveTournamentShortCode, toNum } from '../utils.js';
...
/**
 * Factory, not itself a processor — returns a {@link import('../types.js').GraphicProcessor}
 * bound to `commandKey` (see `processorRegistry.js`'s HIGHEST_RUNS/TOP_BATTER/etc. entries).
 *
...
export function processInningFigures(snapshot) {
  ...
  // Parse score string "105-0" into components — battingTeam has no separate
  // numeric runs/wickets fields, only the combined display string.
  // `|| 0` (not `?? 0`): an empty/malformed scoreStr parses to NaN, which `??`
  // would not catch, showing "NaN" on the broadcast overlay.
  const scoreStr = live.battingTeam?.score ?? '';
  const [runsStr, wktsStr] = scoreStr.split('-');
  const runs = Number.parseInt(runsStr, 10) || 0;
  const wickets = Number.parseInt(wktsStr, 10) || 0;
...
/** @type {Record<string, keyof import('../types.js').GraphicTournamentAggregates>} */
