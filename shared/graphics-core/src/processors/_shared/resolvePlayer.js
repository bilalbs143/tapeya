...
/**
 * Raw command payload / player-ish record from the API. Shape varies per command
 * (each of ~95 commands may send different fields), so this is deliberately loose —
 * `Record<string, any>` opts out of property-level checking here while the rest of
 * the module stays strict against the normalized {@link GraphicSessionSnapshot}.
 * @typedef {Record<string, any>} RawPlayerPayload
 */

/**
 * @param {RawPlayerPayload} [p]
...
/**
 * @param {{ runs?: unknown, balls?: unknown, dots?: unknown, ones?: unknown, twos?: unknown, threes?: unknown, fours?: unknown, sixes?: unknown }} stats
 */
...
/**
 * @param {{ runs?: unknown, balls?: unknown, dots?: unknown, wickets?: unknown, extras?: unknown, economyDisplay?: unknown }} stats
 */
...
/**
 * @param {import('../../types.js').GraphicMatchSnapshot} match
...
export function buildBatsmanMatchProps(snapshot) {
  const p = /** @type {RawPlayerPayload} */ (snapshot.payload ?? {});
  ...
  const isDismissed = !!row?.isDismissed;
