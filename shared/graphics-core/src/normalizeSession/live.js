...
function normalizeWinProbability(wp) {
  ...
  const w = /** @type {Record<string, any>} */ (wp);
  const home = Number(w.home);
  const away = Number(w.away);
 ...
 * @returns {import('../types.js').GraphicAtStageMirror | null}
 */
function normalizeAtStageMirror(mirror) {
  if (!mirror || typeof mirror !== 'object') return null;
  const m = /** @type {Record<string, any>} */ (mirror);
  if (m.batting_team !== 'home' && m.batting_team !== 'away') return null;

  return {
    battingTeam: m.batting_team,
    score: m.score ?? '',
    overs: m.overs ?? '',
    batters: normalizeBatters(m.batters),
    bowler: {
      name: m.bowler?.name ?? '',
      figures: m.bowler?.figures ?? '',
      overs: m.bowler?.overs ?? '',
      imageUrl: m.bowler?.avatar_url ?? m.bowler?.image_url ?? null,
    },
    currentOverDeliveries: normalizeDeliveries(m.current_over_deliveries),
    inningsLabel: m.innings_label ?? '1st Innings',
  };
}

/**
 * @param {Record<string, any>} ctx
 * @param {{ home_team: Record<string, any>, away_team: Record<string, any> }} teams
...
export function normalizeLive(ctx, teams) {
  ...
  return {
    ...
    projectedScore: ctx.projected_score ?? null,
