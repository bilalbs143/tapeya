...
import { matchStreamStatusLabel } from '../match-stream.utils';

import { MatchStreamService, type MatchStreamPayload } from 'src/app/services/match-stream.service';
import { MessageService } from 'src/app/services/message.service';
import { DialogWrapperComponent } from 'src/app/shared/components/dialog-wrapper/dialog-wrapper.component';
import { SubmitButtonComponent } from 'src/app/shared/components/submit-button/submit-button.component';

export interface MatchStreamDialogData {
  matchId: number;
  homeTeamName: string;
  awayTeamName: string;
...
@Component({
  selector: 'app-match-stream-dialog',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './match-stream-dialog.component.html',
})
export class MatchStreamDialogComponent implements OnInit {
  private readonly dialogRef = inject(MatDialogRef<MatchStreamDialogComponent>);
  private readonly streamApi = inject(MatchStreamService);
  private readonly messageService = inject(MessageService);
  private readonly snackBar = inject(MatSnackBar);
  public readonly data = inject<MatchStreamDialogData>(MAT_DIALOG_DATA);
  ...
  public syncStatus(): void {
    this.runAction(
      ...
      ({ status }) => {
        ...
        this.notifyStreamMutated();
        this.messageService.success(`Status Synced: ${matchStreamStatusLabel(status)}`);
      }
    );
  }
